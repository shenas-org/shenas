"""Transformation plugin ABC and TransformationConfig base."""
...
@dataclass
class TransformationConfig(SingletonTable):
    """Base configuration for transformation plugins.

    Transforms that need persistent config (API keys, model names, etc.)
    subclass this. ``Transformation.__init_subclass__`` auto-sets the
    ...
    class _Meta(SingletonTable._Meta):
        display_name = "Transformation Config"
        description = "Per-transformation-plugin configuration."
        pk = ("id",)
        schema = "config"
...
class Transformation(Plugin):
    ...
    A transformation plugin defines a *type* of data transformation
    ...
    Config: ClassVar[type] = TransformationConfig

    def __init_subclass__(cls, **kwargs: Any) -> None:
        super().__init_subclass__(**kwargs)
        if not hasattr(cls, "name"):
            return
        if cls.Config is not TransformationConfig:
    ...
    def has_config(self) -> bool:
        return self.Config is not TransformationConfig
    ...
    def execute(
        ...
        *,
        ...
        The contract:
        1. DELETE FROM target WHERE source = instance.source_plugin
        2. Compute result
    ...
    def param_schema(self) -> list[dict[str, Any]]:
        ...
        ``[{"name": "sql", "type": "textarea", "required": True, "description": "..."}]``
        """
        return []
    ...
    def seed_defaults_for_source(self, source_name: str) -> None:
        ...
        Called during sync. Default is a no-op. The SQL plugin overrides
        this to load transforms.json.
        """
