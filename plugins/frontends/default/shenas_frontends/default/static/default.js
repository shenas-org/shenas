var te=Object.defineProperty;var se=(l,e,t)=>e in l?te(l,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):l[e]=t;var h=(l,e,t)=>se(l,typeof e!="symbol"?e+"":e,t);import{LitElement as v,css as g,html as n}from"lit";import G,{dagre as ae}from"cytoscape";import{Router as ie}from"@lit-labs/router";class z extends v{constructor(){super(),this.open=!1,this.commands=[],this._query="",this._filtered=[],this._selectedIndex=0}updated(e){e.has("open")&&this.open&&(this._query="",this._selectedIndex=0,this._filter(),requestAnimationFrame(()=>{const t=this.renderRoot.querySelector("input");t&&t.focus()})),e.has("commands")&&this._filter()}_filter(){const e=this._query.toLowerCase();e?this._filtered=this.commands.filter(t=>t.label.toLowerCase().includes(e)||t.category.toLowerCase().includes(e)||(t.description||"").toLowerCase().includes(e)):this._filtered=this.commands,this._selectedIndex>=this._filtered.length&&(this._selectedIndex=Math.max(0,this._filtered.length-1))}_onInput(e){this._query=e.target.value,this._selectedIndex=0,this._filter()}_onKeydown(e){if(e.key==="ArrowDown")e.preventDefault(),this._filtered.length>0&&(this._selectedIndex=Math.min(this._selectedIndex+1,this._filtered.length-1)),this._scrollToSelected();else if(e.key==="ArrowUp")e.preventDefault(),this._selectedIndex=Math.max(this._selectedIndex-1,0),this._scrollToSelected();else if(e.key==="Enter"){e.preventDefault();const t=this._filtered[this._selectedIndex];t&&this._execute(t)}else e.key==="Escape"&&this._close()}_scrollToSelected(){requestAnimationFrame(()=>{const e=this.renderRoot.querySelector(".item.selected");e&&e.scrollIntoView({block:"nearest"})})}_execute(e){this.dispatchEvent(new CustomEvent("execute",{detail:e,bubbles:!0,composed:!0}))}_close(){this.dispatchEvent(new CustomEvent("close",{bubbles:!0,composed:!0}))}render(){return this.open?n`
    ...
    `:n``}}h(z,"properties",{open:{type:Boolean,reflect:!0},commands:{type:Array},_query:{state:!0},_filtered:{state:!0},_selectedIndex:{state:!0}}),h(z,"styles",g`
    :host {
      display: none;
    }
    :host([open]) {
  ...
  `);customElements.define("shenas-command-palette",z);const Q=g`
  table {
    width: 100%;
...
`;class B extends v{constructor(){super(),this.loading=!1,this.empty=!1,this.loadingText="Loading...",this.emptyText="No data",this.displayName=""}updated(e){e.has("displayName")&&this.displayName&&this.dispatchEvent(new CustomEvent("page-title",{bubbles:!0,composed:!0,detail:{title:this.displayName}}))}render(){return this.loading?n`<p class="loading">${this.loadingText}</p>`:this.empty?n`<p class="empty">${this.emptyText}</p>`:n`<slot></slot>`}}h(B,"properties",{loading:{type:Boolean,reflect:!0},empty:{type:Boolean,reflect:!0},loadingText:{type:String,attribute:"loading-text"},emptyText:{type:String,attribute:"empty-text"},displayName:{type:String,attribute:"display-name"}}),h(B,"styles",[D,g`
    ...
    `]);customElements.define("shenas-page",B);class O extends v{constructor(){super(),this.enabled=!1,this.toggleable=!1}updated(){this.title=this.enabled?"Enabled":"Disabled"}render(){return n`<div class="track" @click=${this._onClick}><div class="knob"></div></div>`}_onClick(){this.toggleable&&this.dispatchEvent(new CustomEvent("toggle",{bubbles:!0,composed:!0}))}}h(O,"properties",{enabled:{type:Boolean,reflect:!0},toggleable:{type:Boolean,reflect:!0}}),h(O,"styles",g`
    :host {
  ...
  `);customElements.define("status-toggle",O);class A extends v{constructor(){super(),this.columns=[],this.rows=[],this.rowClass=null,this.actions=null,this.emptyText="No items",this.showAdd=!1}_onAdd(){this.dispatchEvent(new CustomEvent("add",{bubbles:!0,composed:!0}))}render(){const e=typeof this.actions=="function",t=this.showAdd?n`<div class="add-row"><button class="add-btn" title="Add" @click=${this._onAdd}>+</button></div>`:"";return!this.rows||this.rows.length===0?n`<p class="empty">${this.emptyText}</p>${t}`:n`
      ...
      ${t}
    `}}h(A,"properties",{columns:{type:Array},rows:{type:Array},rowClass:{type:Object},actions:{type:Object},emptyText:{type:String,attribute:"empty-text"},showAdd:{type:Boolean,attribute:"show-add"}}),h(A,"styles",[Q,x,N,D,g`
      :host {
        display: block;
      }
      .mono {
    ...
    `]);customElements.define("shenas-data-list",A);class L extends v{constructor(){super(),this.title="",this.submitLabel="Save"}render(){return n`
    ...
    `}_onSubmit(){this.dispatchEvent(new CustomEvent("submit",{bubbles:!0,composed:!0}))}_onCancel(){this.dispatchEvent(new CustomEvent("cancel",{bubbles:!0,composed:!0}))}}h(L,"properties",{title:{type:String},submitLabel:{type:String,attribute:"submit-label"}}),h(L,"styles",[x,g`
      :host {
        display: block;
        margin: 1rem 0;
        padding: 1rem;
    ...
    `]);customElements.define("shenas-form-panel",L);const re="modulepreload",oe=function(l){return"/"+l},Y={},le=function(e,t,s){let a=Promise.resolve();if(t&&t.length>0){let r=function(p){return Promise.all(p.map(b=>Promise.resolve(b).then(u=>({status:"fulfilled",value:u}),u=>({status:"rejected",reason:u}))))};document.getElementsByTagName("link");const o=document.querySelector("meta[property=csp-nonce]"),c=(o==null?void 0:o.nonce)||(o==null?void 0:o.getAttribute("nonce"));a=r(t.map(p=>{if(p=oe(p),p in Y)return;Y[p]=!0;const b=p.endsWith(".css"),u=b?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${p}"]${u}`))return;const d=document.createElement("link");if(d.rel=b?"stylesheet":re,b||(d.as="script"),d.crossOrigin="",d.href=p,c&&d.setAttribute("nonce",c),document.head.appendChild(d),b)return new Promise((m,k)=>{d.addEventListener("load",m),d.addEventListener("error",()=>k(new Error(`Unable to preload CSS for ${p}`)))})}))}function i(r){const o=new Event("vite:preloadError",{cancelable:!0});if(o.payload=r,window.dispatchEvent(o),!o.defaultPrevented)throw r}return a.then(r=>{for(const o of r||[])o.status==="rejected"&&i(o.reason);return e().catch(i)})};async function _(l,e,t={}){const s=await fetch(`${l}/graphql`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({query:e,variables:t})});if(!s.ok)return null;const a=await s.json();return a.errors?(console.warn("GraphQL errors:",a.errors),null):a.data}async function f(l,e,t={}){const s=await fetch(`${l}/graphql`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({query:e,variables:t})});if(!s.ok)return{ok:!1,data:null,errors:[{message:`HTTP ${s.status}`}]};const a=await s.json();return{ok:!a.errors,data:a.data,errors:a.errors||[]}}async function E(l,e){const{tableFromIPC:t}=await le(async()=>{const{tableFromIPC:r}=await import("/vendor/apache-arrow.js");return{tableFromIPC:r}},[]),s=await fetch(`${l}/query?sql=${encodeURIComponent(e)}`);if(!s.ok)return null;const a=await s.arrayBuffer();return t(new Uint8Array(a)).toArray().map(r=>r.toJSON())}function P(l){return l?n`<div class="message ${l.type}">${l.text}</div>`:""}function ee(l,e,t){l.dispatchEvent(new CustomEvent("register-command",{bubbles:!0,composed:!0,detail:{componentId:e,commands:t}}))}let Z=!1;class q extends v{constructor(){super(),this.apiBase="/api",this._loading=!0,this._empty=!1,this._cy=null,this._elements=null,this._resizeObserver=null}connectedCallback(){super.connectedCallback(),this._fetchData()}disconnectedCallback(){super.disconnectedCallback(),this._cy&&(this._cy.destroy(),this._cy=null),this._resizeObserver&&(this._resizeObserver.disconnect(),this._resizeObserver=null)}async _fetchData(){this._loading=!0;try{const e=await _(this.apiBase,`{
        transforms { id sourceDuckdbSchema sourceDuckdbTable targetDuckdbSchema targetDuckdbTable sourcePlugin enabled }
        dependencies
      }`),t=this.allPlugins||{};this._buildElements(t.pipe||[],t.schema||[],(e==null?void 0:e.transforms)||[],this.schemaPlugins||{},t.component||[],(e==null?void 0:e.dependencies)||{})}catch(e){console.error("Failed to fetch overview data:",e)}this._loading=!1}_buildElements(e,t,s,a,i,r){const o=[],c=new Set,p={};for(const[d,m]of Object.entries(a))for(const k of m)p[k]=d;for(const d of e){const m=`pipe:${d.name}`;c.add(m),o.push({data:{id:m,label:d.displayName||d.name,kind:"source",enabled:d.enabled!==!1?"yes":"no"}})}for(const d of t){const m=`schema:${d.name}`;c.add(m),o.push({data:{id:m,label:d.displayName||d.name,kind:"dataset",enabled:d.enabled!==!1?"yes":"no"}})}for(const d of i){const m=`component:${d.name}`;c.add(m),o.push({data:{id:m,label:d.displayName||d.name,kind:"dashboard",enabled:d.enabled!==!1?"yes":"no"}})}for(const d of s){const m=`pipe:${d.sourcePlugin}`,k=p[d.targetDuckdbTable],T=k?`schema:${k}`:null;if(!T||!c.has(m)||!c.has(T))continue;const $=d.description||`${d.sourceDuckdbTable} -> ${d.targetDuckdbTable}`,w=$.length>30?$.slice(0,28)+"...":$;o.push({data:{id:`transform:${d.id}`,source:m,target:T,label:w,enabled:d.enabled?"yes":"no",sourcePlugin:d.sourcePlugin,edgeType:"transform"}})}const b=new Set;for(const d of o)d.data.edgeType==="transform"&&b.add(`${d.data.source}:${d.data.target}`);const u=new Set;for(const[d,m]of Object.entries(r))for(const k of m){const T=d.split(":")[0];let $,w;if(T==="dashboard"?($=k,w=d):($=d,w=k),!c.has($)||!c.has(w)||T==="source"&&b.has(`${$}:${w}`))continue;const R=`dep:${$}:${w}`;u.has(R)||(u.add(R),o.push({data:{id:R,source:$,target:w,edgeType:"dependency"}}))}this._elements=o,this._empty=o.filter(d=>d.data.source).length===0}_initCytoscape(){const e=this.renderRoot.querySelector("#cy");!e||!this._elements||(Z||(G.use(ae),Z=!0),this._cy&&this._cy.destroy(),this._cy=G({container:e,elements:this._elements,style:[{selector:"node",style:{label:"data(label)","text-valign":"center","text-halign":"center","font-size":12,color:"#fff","text-wrap":"wrap","text-max-width":100,width:120,height:40,shape:"round-rectangle"}},{selector:'node[kind="source"]',style:{"background-color":"#4a90d9",cursor:"pointer"}},{selector:'node[kind="dataset"]',style:{"background-color":"#66bb6a",cursor:"pointer"}},{selector:'node[kind="dashboard"]',style:{"background-color":"#ffa726",cursor:"pointer"}},{selector:'node[enabled="no"]',style:{opacity:.4,"border-width":2,"border-color":"#999","border-style":"dashed"}},{selector:"edge",style:{"curve-style":"bezier","target-arrow-shape":"triangle","target-arrow-color":"#999","line-color":"#999",cursor:"pointer",width:2,label:"data(label)","font-size":9,color:"#888","text-rotation":"autorotate","text-margin-y":-8}},{selector:'edge[enabled="yes"]',style:{"line-style":"solid"}},{selector:'edge[enabled="no"]',style:{"line-style":"dashed","line-color":"#ccc","target-arrow-color":"#ccc",opacity:.5}},{selector:'edge[edgeType="dependency"]',style:{"line-style":"dotted","line-color":"#bbb","target-arrow-color":"#bbb",width:1.5,label:""}}],layout:{name:"dagre",rankDir:"LR",nodeSep:60,rankSep:150,padding:30},userZoomingEnabled:!0,userPanningEnabled:!0,boxSelectionEnabled:!1}),this._cy.on("tap","node",t=>{const s=t.target.data(),a=s.id.substring(s.id.indexOf(":")+1);let i;if(s.kind==="source")i=`/settings/source/${a}`;else if(s.kind==="dataset")i=`/settings/dataset/${a}`;else if(s.kind==="dashboard")i=`/settings/dashboard/${a}`;else return;this.dispatchEvent(new CustomEvent("navigate",{bubbles:!0,composed:!0,detail:{path:i}}))}),this._cy.on("tap","edge",t=>{const s=t.target.data("sourcePlugin");s&&this.dispatchEvent(new CustomEvent("navigate",{bubbles:!0,composed:!0,detail:{path:`/settings/source/${s}`}}))}),this._resizeObserver&&this._resizeObserver.disconnect(),this._resizeObserver=new ResizeObserver(()=>{this._cy&&(this._cy.resize(),this._cy.fit(void 0,30))}),this._resizeObserver.observe(e))}firstUpdated(){!this._loading&&this._elements&&this._initCytoscape()}updated(e){e.has("_loading")&&!this._loading&&this._elements&&requestAnimationFrame(()=>this._initCytoscape())}render(){return n`
    ...
    `}}h(q,"properties",{apiBase:{type:String,attribute:"api-base"},allPlugins:{type:Object},schemaPlugins:{type:Object},_loading:{state:!0},_empty:{state:!0}}),h(q,"styles",[D,g`
      :host {
        display: block;
      }
      #cy {
    ...
    `}}h(F,"properties",{apiBase:{type:String,attribute:"api-base"},pipeName:{type:String,attribute:"pipe-name"},_fields:{state:!0},_instructions:{state:!0},_loading:{state:!0},_message:{state:!0},_needsMfa:{state:!0},_oauthUrl:{state:!0},_submitting:{state:!0},_stored:{state:!0}}),h(F,"styles",[x,I,C,g`
      :host {
        display: block;
      }
      .instructions {
    ...
    `]);customElements.define("shenas-auth",F);const y=class y extends v{constructor(){super(),this.apiBase="/api",this.kind="",this.name="",this._config=null,this._loading=!0,this._message=null,this._editing=null,this._editValue="",this._freqNum="",this._freqUnit="hours"}willUpdate(e){(e.has("kind")||e.has("name"))&&this._fetchConfig()}async _fetchConfig(){if(!this.kind||!this.name)return;this._loading=!0;const e=await _(this.apiBase,"query($kind: String, $name: String) { config(kind: $kind, name: $name) { kind name entries { key label value description } } }",{kind:this.kind,name:this.name}),t=(e==null?void 0:e.config)||[];this._config=t.length>0?t[0]:null,this._loading=!1}_startEdit(e,t){if(this._editing=e,this._editValue=t||"",y._DURATION_FIELDS.has(e)&&t){const s=parseFloat(t);s>=1440&&s%1440===0?(this._freqNum=String(s/1440),this._freqUnit="days"):s>=60&&s%60===0?(this._freqNum=String(s/60),this._freqUnit="hours"):s>=1?(this._freqNum=String(s),this._freqUnit="minutes"):(this._freqNum=String(s*60),this._freqUnit="seconds")}else y._DURATION_FIELDS.has(e)&&(this._freqNum="",this._freqUnit="hours")}_cancelEdit(){this._editing=null,this._editValue=""}_freqToMinutes(){const e=parseFloat(this._freqNum);return isNaN(e)||e<=0?null:String(Math.round(e*y._UNIT_MULTIPLIERS[this._freqUnit]))}_formatFreq(e){const t=parseFloat(e);return isNaN(t)?e:t>=1440&&t%1440===0?`${t/1440} day${t/1440!==1?"s":""}`:t>=60&&t%60===0?`${t/60} hour${t/60!==1?"s":""}`:t>=1?`${t} minute${t!==1?"s":""}`:`${t*60} second${t*60!==1?"s":""}`}async _saveEdit(e){const t=y._DURATION_FIELDS.has(e)?this._freqToMinutes():this._editValue;if(y._DURATION_FIELDS.has(e)&&t===null){this._message={type:"error",text:"Enter a positive number"};return}const{ok:s,data:a}=await f(this.apiBase,"mutation($kind: String!, $name: String!, $key: String!, $value: String!) { setConfig(kind: $kind, name: $name, key: $key, value: $value) { ok } }",{kind:this.kind,name:this.name,key:e,value:t});s?(this._message={type:"success",text:`Updated ${e}`},this._editing=null,await this._fetchConfig()):this._message={type:"error",text:(a==null?void 0:a.detail)||"Update failed"}}render(){var t;const e=!this._config||this._config.entries.length===0;return n`
      <shenas-page ?loading=${this._loading} ?empty=${e}
    ...
    `}_renderFreqEdit(e){return n`
      <div class="edit-row">
        ...
        <select @change=${t=>{this._freqUnit=t.target.value}}>
          ${Object.keys(y._UNIT_MULTIPLIERS).map(t=>n`
      ...
      </div>`}_renderEntry(e){const t=this._editing===e.key,s=y._DURATION_FIELDS.has(e.key),a=s&&e.value?this._formatFreq(e.value):e.value;return n`
    ...
    `}};h(y,"properties",{apiBase:{type:String,attribute:"api-base"},kind:{type:String},name:{type:String},_config:{state:!0},_loading:{state:!0},_message:{state:!0},_editing:{state:!0},_editValue:{state:!0},_freqNum:{state:!0},_freqUnit:{state:!0}}),h(y,"styles",[x,I,C,g`
      :host {
        display: block;
      }
      .config-row {
    ...
    `]),h(y,"_UNIT_MULTIPLIERS",{seconds:1/60,minutes:1,hours:60,days:1440}),h(y,"_DURATION_FIELDS",new Set(["sync_frequency","lookback_period"]));let j=y;customElements.define("shenas-config",j);function de(l){if(!l)return null;const e=l.split("+").map(t=>t.trim().toLowerCase());return{ctrl:e.includes("ctrl")||e.includes("cmd"),shift:e.includes("shift"),alt:e.includes("alt"),key:e.filter(t=>!["ctrl","cmd","shift","alt"].includes(t))[0]||""}}function ce(l){const e=[];(l.ctrlKey||l.metaKey)&&e.push("Ctrl"),l.shiftKey&&e.push("Shift"),l.altKey&&e.push("Alt");const t=l.key.length===1?l.key.toUpperCase():l.key;return["Control","Shift","Alt","Meta"].includes(l.key)||e.push(t),e.join("+")}function he(l,e){const t=de(e);return!t||!t.key?!1:(l.ctrlKey||l.metaKey)===t.ctrl&&l.shiftKey===t.shift&&l.altKey===t.alt&&l.key.toLowerCase()===t.key}const S=[{id:"source",label:"Pipes"},{id:"dataset",label:"Schemas"},{id:"dashboard",label:"Components"},{id:"ui",label:"UI"},{id:"theme",label:"Themes"}];class U extends v{constructor(){super(),this.apiBase="/api",this.actions=[],this._bindings={},this._recording=null,this._recordedKey="",this._conflict=null,this._loading=!0,this._filter=""}connectedCallback(){super.connectedCallback(),this._loadBindings(),this._boundKeydown=e=>this._onKeydown(e),document.addEventListener("keydown",this._boundKeydown,!0)}disconnectedCallback(){super.disconnectedCallback(),document.removeEventListener("keydown",this._boundKeydown,!0)}async _loadBindings(){this._loading=!0;const e=await _(this.apiBase,"{ hotkeys }");this._bindings=(e==null?void 0:e.hotkeys)||{},this._loading=!1}async _saveBinding(e,t){t?await f(this.apiBase,"mutation($id: String!, $b: String!) { setHotkey(actionId: $id, binding: $b) { ok } }",{id:e,b:t}):await f(this.apiBase,"mutation($id: String!) { deleteHotkey(actionId: $id) { ok } }",{id:e}),this.dispatchEvent(new CustomEvent("hotkeys-changed",{bubbles:!0,composed:!0}))}_startRecording(e){this._recording=e,this._recordedKey="",this._conflict=null}_stopRecording(){this._recording=null,this._recordedKey="",this._conflict=null}_onKeydown(e){if(!this._recording)return;if(e.preventDefault(),e.stopPropagation(),e.key==="Escape"){this._stopRecording();return}if(["Control","Shift","Alt","Meta"].includes(e.key))return;const t=ce(e);this._recordedKey=t;const s=Object.entries(this._bindings).find(([a,i])=>i===t&&a!==this._recording);this._conflict=s?s[0]:null}async _applyRecording(){!this._recordedKey||!this._recording||(this._conflict&&(this._bindings={...this._bindings,[this._conflict]:""},await this._saveBinding(this._conflict,"")),this._bindings={...this._bindings,[this._recording]:this._recordedKey},await this._saveBinding(this._recording,this._recordedKey),this._stopRecording())}async _clearBinding(e){this._bindings={...this._bindings,[e]:""},await this._saveBinding(e,"")}async _resetDefaults(){await f(this.apiBase,"mutation { resetHotkeys { ok } }"),await this._loadBindings(),this.dispatchEvent(new CustomEvent("hotkeys-changed",{bubbles:!0,composed:!0}))}_getActionLabel(e){const t=this.actions.find(s=>s.id===e);return t?t.label:e}_getActionCategory(e){const t=this.actions.find(s=>s.id===e);return t?t.category:""}render(){if(this._loading)return n`<p class="loading">Loading hotkeys...</p>`;const e=this._filter.toLowerCase(),t=this.actions.filter(s=>!e||s.label.toLowerCase().includes(e)||s.category.toLowerCase().includes(e));return n`
      <div class="toolbar">
    ...
    `}}h(U,"properties",{apiBase:{type:String,attribute:"api-base"},actions:{type:Array},_bindings:{state:!0},_recording:{state:!0},_recordedKey:{state:!0},_conflict:{state:!0},_loading:{state:!0},_filter:{state:!0}}),h(U,"styles",[x,C,D,g`
      :host {
        display: block;
      }
      .toolbar {
    ...
    `}_formatTime(e){if(!e)return"-";const t=typeof e=="number"?new Date(e):new Date(String(e).endsWith("Z")?e:e+"Z");if(isNaN(t))return String(e);const s=(a,i=2)=>String(a).padStart(i,"0");return`${t.getFullYear()}-${s(t.getMonth()+1)}-${s(t.getDate())} ${s(t.getHours())}:${s(t.getMinutes())}:${s(t.getSeconds())}`}}h(M,"properties",{apiBase:{type:String,attribute:"api-base"},pipe:{type:String},_activeTab:{state:!0},_logs:{state:!0},_spans:{state:!0},_loading:{state:!0},_search:{state:!0},_severity:{state:!0},_expanded:{state:!0},_live:{state:!0}}),h(M,"styles",[x,D,g`
      :host {
        display: flex;
        flex-direction: column;
        height: 100%;
    ...
    `]);customElements.define("shenas-logs",M);class K extends v{constructor(){super();h(this,"_router",new ie(this,[{path:"/",render:()=>this._renderDynamicHome()},{path:"/settings",render:()=>this._renderSettings("flow")},{path:"/settings/:kind",render:({kind:t})=>this._renderSettings(t)},{path:"/settings/:kind/:name",render:({kind:t,name:s})=>this._renderPluginDetail(t,s)},{path:"/settings/:kind/:name/config",render:({kind:t,name:s})=>this._renderPluginDetail(t,s,"config")},{path:"/settings/:kind/:name/auth",render:({kind:t,name:s})=>this._renderPluginDetail(t,s,"auth")},{path:"/settings/:kind/:name/data",render:({kind:t,name:s})=>this._renderPluginDetail(t,s,"data")},{path:"/settings/:kind/:name/logs",render:({kind:t,name:s})=>this._renderPluginDetail(t,s,"logs")},{path:"/logs",render:()=>n`<shenas-logs api-base="${this.apiBase}"></shenas-logs>`},{path:"/:tab",render:({tab:t})=>this._renderDynamicTab(t)}]));h(this,"_hotkeys",{});h(this,"_pluginDisplayNames",{});h(this,"_nextTabId",1);h(this,"_saveWorkspaceTimer",null);this.apiBase="/api",this._components=[],this._loading=!0,this._loadedScripts=new Set,this._elementCache=new Map,this._leftWidth=160,this._rightWidth=220,this._dbStatus=null,this._inspectTable=null,this._inspectRows=null,this._paletteOpen=!1,this._paletteCommands=[],this._navPaletteOpen=!1,this._navCommands=[],this._registeredCommands=new Map,this._tabs=[],this._activeTabId=null,this._allPlugins={}}connectedCallback(){super.connectedCallback(),this._fetchData(),this.addEventListener("plugin-state-changed",()=>this._refreshComponents()),this.addEventListener("job-start",t=>{var s;return(s=this._getJobPanel())==null?void 0:s.addJob(t.detail.id,t.detail.label)}),this.addEventListener("job-log",t=>{var s;return(s=this._getJobPanel())==null?void 0:s.appendLine(t.detail.id,t.detail.text)}),this.addEventListener("job-finish",t=>{var s;return(s=this._getJobPanel())==null?void 0:s.finishJob(t.detail.id,t.detail.ok,t.detail.message)}),this.addEventListener("inspect-table",t=>this._inspect(t.detail.schema,t.detail.table)),this.addEventListener("page-title",t=>{this._activeTabId!=null&&(this._tabs=this._tabs.map(s=>s.id===this._activeTabId?{...s,label:t.detail.title}:s))}),this.addEventListener("navigate",t=>this._navigateTo(t.detail.path,t.detail.label)),this.addEventListener("register-command",t=>{const{componentId:s,commands:a}=t.detail;!a||a.length===0?this._registeredCommands.delete(s):this._registeredCommands.set(s,a)}),this._keyHandler=t=>{for(const[s,a]of Object.entries(this._hotkeys))if(a&&he(t,a))for(const i of this._registeredCommands.values()){const r=i.find(o=>o.id===s);if(r&&r.action){t.preventDefault(),r.action();return}}},document.addEventListener("keydown",this._keyHandler),this.addEventListener("hotkeys-changed",()=>this._loadHotkeys())}disconnectedCallback(){super.disconnectedCallback(),document.removeEventListener("keydown",this._keyHandler)}async _loadHotkeys(){const t=await _(this.apiBase,"{ hotkeys }");this._hotkeys=(t==null?void 0:t.hotkeys)||{}}_togglePalette(){if(this._paletteOpen){this._paletteOpen=!1;return}this._navPaletteOpen=!1,this._buildCommands(),this._paletteOpen=!0}async _toggleNavPalette(){if(this._navPaletteOpen){this._navPaletteOpen=!1;return}this._paletteOpen=!1,await this._buildNavCommands(),this._navPaletteOpen=!0}async _buildNavCommands(){const t=[];for(const a of this._components)t.push({id:`nav:${a.name}`,category:"Page",label:a.display_name||a.name,path:`/${a.name}`});t.push({id:"nav:dataflow",category:"Settings",label:"Flow",path:"/settings/flow"});for(const a of S)t.push({id:`nav:settings:${a.id}`,category:"Settings",label:a.label,path:`/settings/${a.id}`});const s=S.flatMap(a=>(this._allPlugins[a.id]||[]).map(i=>({...i,kind:a.id,kindLabel:a.label})));for(const a of s)t.push({id:`nav:${a.kind}:${a.name}`,category:a.kindLabel,label:a.displayName||a.name,path:`/settings/${a.kind}/${a.name}`});this._navCommands=t}async _registerGlobalCommands(){const t=[],s={};try{const a=this._schemaPlugins||{};for(const i of S){const r=this._allPlugins[i.id]||[];for(const o of r){const c=o.displayName||o.name;s[`${i.id}:${o.name}`]=c;const p=o.enabled!==!1;t.push({id:`toggle:${i.id}:${o.name}`,category:i.label,label:`Toggle ${c}`,action:async()=>{const b=p?"mutation($k: String!, $n: String!) { disablePlugin(kind: $k, name: $n) { ok } }":"mutation($k: String!, $n: String!) { enablePlugin(kind: $k, name: $n) { ok } }";await f(this.apiBase,b,{k:i.id,n:o.name}),await this._fetchData()}}),i.id==="source"&&p&&(t.push({id:`sync:${o.name}`,category:"Pipe",label:`Sync ${c}`,action:()=>fetch(`${this.apiBase}/sync/${o.name}`,{method:"POST"})}),t.push({id:`transform:pipe:${o.name}`,category:"Transform",label:`Run Transforms: ${c}`,action:()=>f(this.apiBase,"mutation($pipe: String!) { runPipeTransforms(pipe: $pipe) }",{pipe:o.name})}))}}t.push({id:"sync:all",category:"Pipe",label:"Sync All Pipes",action:()=>fetch(`${this.apiBase}/sync`,{method:"POST"})}),t.push({id:"seed:transforms",category:"Transform",label:"Seed Default Transforms",action:()=>f(this.apiBase,"mutation { seedTransforms }")});for(const i of this._allPlugins.schema||[]){const r=a[i.name]||[];for(const o of r)t.push({id:`transform:schema:${o}`,category:"Transform",label:`Run Transforms -> ${i.displayName||i.name}: ${o}`,action:()=>f(this.apiBase,"mutation($schema: String!) { runSchemaTransforms(schema: $schema) }",{schema:o})})}}catch{}this._pluginDisplayNames=s,t.push({id:"command-palette",category:"System",label:"Command Palette",action:()=>this._togglePalette()},{id:"navigation-palette",category:"System",label:"Navigation Palette",action:()=>this._toggleNavPalette()},{id:"close-tab",category:"System",label:"Close Tab",action:()=>{this._activeTabId!=null&&this._closeTab(this._activeTabId)}},{id:"new-tab",category:"System",label:"New Tab",action:()=>this._addTab()}),this._registeredCommands.set("global",t)}_buildCommands(){const t=[];for(const s of this._registeredCommands.values())t.push(...s);this._paletteCommands=t}_executePaletteCommand(t){const s=t.detail;s.path?this._openTab(s.path,s.label):s.action&&s.action(),this._paletteOpen=!1,this._navPaletteOpen=!1}_navigateTo(t,s){if(this._tabs.length===0||!this._activeTabId){this._openTab(t,s);return}const a=s||this._labelForPath(t);this._tabs=this._tabs.map(i=>i.id===this._activeTabId?{...i,path:t,label:a}:i),window.history.pushState({},"",t),this._router.goto(t),this._saveWorkspace()}_openTab(t,s){const a=this._nextTabId++;this._tabs=[...this._tabs,{id:a,path:t,label:s||this._labelForPath(t)}],this._activeTabId=a,window.history.pushState({},"",t),this._router.goto(t),this._saveWorkspace()}async _addTab(){await this._buildNavCommands(),this._navPaletteOpen=!0}_closeTab(t){const s=this._tabs.findIndex(i=>i.id===t);if(s===-1)return;const a=this._tabs.filter(i=>i.id!==t);if(this._tabs=a,this._activeTabId===t)if(a.length>0){const i=a[Math.min(s,a.length-1)];this._activeTabId=i.id,this._router.goto(i.path)}else this._activeTabId=null,window.history.pushState({},"","/");this._saveWorkspace()}_switchTab(t){const s=this._tabs.find(a=>a.id===t);s&&(this._activeTabId=t,window.history.pushState({},"",s.path),this._router.goto(s.path),this._saveWorkspace())}_saveWorkspace(){clearTimeout(this._saveWorkspaceTimer),this._saveWorkspaceTimer=setTimeout(()=>{const t={tabs:this._tabs,activeTabId:this._activeTabId,nextTabId:this._nextTabId};f(this.apiBase,"mutation($data: JSON!) { saveWorkspace(data: $data) { ok } }",{data:t}).catch(()=>{})},300)}async _loadWorkspace(){try{const t=await _(this.apiBase,"{ workspace }"),s=t==null?void 0:t.workspace;if(!s)return;if(s.tabs&&s.tabs.length>0){this._tabs=s.tabs,this._activeTabId=s.activeTabId||s.tabs[0].id,this._nextTabId=s.nextTabId||Math.max(...s.tabs.map(r=>r.id))+1;const a=window.location.pathname;if(a&&a!=="/"&&!this._tabs.some(r=>r.path===a)){this._openTab(a);return}const i=this._tabs.find(r=>r.id===this._activeTabId);i&&this._router.goto(i.path)}else{const a=window.location.pathname;a&&a!=="/"&&this._openTab(a)}}catch{const t=window.location.pathname;t&&t!=="/"&&this._openTab(t)}}_labelForPath(t){const s=t.replace(/^\/+/,"");if(!s||s==="settings"||s==="settings/flow")return"Flow";const a=s.split("/");if(a[0]==="settings"){if(a.length===2){const r=S.find(o=>o.id===a[1]);return r?r.label:a[1]}if(a.length>=3){const r=`${a[1]}:${a[2]}`;return this._pluginDisplayNames[r]||a[2]}}const i=this._components.find(r=>r.name===a[0]);return i?i.display_name||i.name:a[0]}async _refreshComponents(){const t=await _(this.apiBase,"{ components }");this._components=(t==null?void 0:t.components)||[]}async _refreshPlugins(){const t=await _(this.apiBase,`{
      ...
      themes: plugins(kind: "theme") { name displayName enabled }
    }`);t&&(this._allPlugins={pipe:t.pipes||[],schema:t.schemas||[],component:t.componentPlugins||[],ui:t.uis||[],theme:t.themes||[]})}async _fetchData(){var t,s;this._loading=!0;try{const a=await _(this.apiBase,`{
        components
        hotkeys
        workspace
        ...
        theme { css }
        schemaPlugins
      }`);if(this._components=(a==null?void 0:a.components)||[],this._dbStatus=a==null?void 0:a.dbStatus,this._hotkeys=(a==null?void 0:a.hotkeys)||{},this._allPlugins={pipe:(a==null?void 0:a.pipes)||[],schema:(a==null?void 0:a.schemas)||[],component:(a==null?void 0:a.componentPlugins)||[],ui:(a==null?void 0:a.uis)||[],theme:(a==null?void 0:a.themes)||[]},this._schemaPlugins=(a==null?void 0:a.schemaPlugins)||{},(t=a==null?void 0:a.theme)!=null&&t.css&&!document.querySelector("link[data-shenas-theme]")){const r=document.createElement("link");r.rel="stylesheet",r.setAttribute("data-shenas-theme",""),r.href=a.theme.css,document.head.appendChild(r)}const i=a==null?void 0:a.workspace;if(((s=i==null?void 0:i.tabs)==null?void 0:s.length)>0){this._tabs=i.tabs,this._activeTabId=i.activeTabId||i.tabs[0].id,this._nextTabId=i.nextTabId||Math.max(...i.tabs.map(o=>o.id))+1;const r=window.location.pathname;if(r&&r!=="/"&&!this._tabs.some(o=>o.path===r))this._openTab(r);else{const o=this._tabs.find(c=>c.id===this._activeTabId);o&&this._router.goto(o.path)}}else{const r=window.location.pathname;r&&r!=="/"&&this._openTab(r)}}catch(a){console.error("Failed to fetch data:",a)}await this._registerGlobalCommands();try{const i=await(await fetch(`${this.apiBase}/auth/me`)).json();this._remoteUser=i.user||null}catch{this._remoteUser=null}this._loading=!1}_activeTab(){var s,a;const t=this._tabs.find(i=>i.id===this._activeTabId);return((a=(s=t==null?void 0:t.path)==null?void 0:s.replace(/^\/+/,""))==null?void 0:a.split("/")[0])||(this._components.length>0?this._components[0].name:"settings")}_activePath(){const t=this._tabs.find(s=>s.id===this._activeTabId);return(t==null?void 0:t.path)||window.location.pathname}_startDrag(t){return s=>{s.preventDefault();const a=s.clientX,i=t==="left"?this._leftWidth:this._rightWidth,r=s.target;r.classList.add("dragging");const o=p=>{const b=t==="left"?p.clientX-a:a-p.clientX,u=Math.max(80,Math.min(400,i+b));t==="left"?this._leftWidth=u:this._rightWidth=u},c=()=>{r.classList.remove("dragging"),window.removeEventListener("mousemove",o),window.removeEventListener("mouseup",c)};window.addEventListener("mousemove",o),window.addEventListener("mouseup",c)}}render(){var a;if(this._loading)return n`<shenas-page loading></shenas-page>`;const t=this._activeTab(),s=this._activePath();return s.startsWith("/settings")&&this._settingsOpen===void 0&&(this._settingsOpen=!0),n`
      <div class="layout">
    ...
    ></shenas-plugin-detail>`}_getAllActions(){const t=new Set,s=[];for(const a of this._registeredCommands.values())for(const i of a)!t.has(i.id)&&i.action&&(t.add(i.id),s.push({id:i.id,label:i.label,category:i.category}));return s.sort((a,i)=>a.category==="System"&&i.category!=="System"?-1:a.category!=="System"&&i.category==="System"?1:0),s}_getJobPanel(){var t;return(t=this.shadowRoot)==null?void 0:t.querySelector("shenas-job-panel")}_renderSettings(t){return n`<shenas-settings
      api-base="${this.apiBase}"
      active-kind="${t||"flow"}"
    ...
    `}_getOrCreateElement(t){if(!this._elementCache.has(t.name)){const s=document.createElement(t.tag);s.setAttribute("api-base",this.apiBase),this._elementCache.set(t.name,s)}return this._elementCache.get(t.name)}}h(K,"properties",{apiBase:{type:String,attribute:"api-base"},_components:{state:!0},_loading:{state:!0},_loadedScripts:{state:!0},_leftWidth:{state:!0},_rightWidth:{state:!0},_dbStatus:{state:!0},_inspectTable:{state:!0},_inspectRows:{state:!0},_paletteOpen:{state:!0},_paletteCommands:{state:!0},_navPaletteOpen:{state:!0},_settingsOpen:{state:!0},_remoteUser:{state:!0},_navCommands:{state:!0},_tabs:{state:!0},_activeTabId:{state:!0},_allPlugins:{state:!0}}),h(K,"styles",[N,D,g`
      :host {
        display: block;
        height: 100vh;
        color: var(--shenas-text, #222);
    ...
    }`),t={pipe:(e==null?void 0:e.pipes)||[],schema:(e==null?void 0:e.schemas)||[],component:(e==null?void 0:e.componentPlugins)||[],ui:(e==null?void 0:e.uis)||[],theme:(e==null?void 0:e.themes)||[]};this._plugins=t,this._loading=!1,this.onPluginsChanged&&this.onPluginsChanged(t)}async _togglePlugin(e,t,s){const a=s?"disable":"enable",i=a==="enable"?"mutation($k: String!, $n: String!) { enablePlugin(kind: $k, name: $n) { ok message } }":"mutation($k: String!, $n: String!) { disablePlugin(kind: $k, name: $n) { ok message } }",{data:r}=await f(this.apiBase,i,{k:e,n:t}),o=a==="enable"?r==null?void 0:r.enablePlugin:r==null?void 0:r.disablePlugin;o!=null&&o.ok||(this._actionMessage={type:"error",text:(o==null?void 0:o.message)||`${a} failed`}),e==="theme"&&await this._applyActiveTheme(),await this._fetchAll({force:!0})}async _applyActiveTheme(){const e=await _(this.apiBase,"{ theme { css } }");if(!(e!=null&&e.theme))return;const{css:t}=e.theme;let s=document.querySelector("link[data-shenas-theme]");t?(s||(s=document.createElement("link"),s.rel="stylesheet",s.setAttribute("data-shenas-theme",""),document.head.appendChild(s)),s.href=t):s&&s.remove()}async _startInstall(e){this._installing=!0,this._selectedPlugin="",this._availablePlugins=null;const t=await _(this.apiBase,"query($kind: String!) { availablePlugins(kind: $kind) }",{kind:e}),s=(t==null?void 0:t.availablePlugins)||[],a=new Set((this._plugins[e]||[]).map(i=>i.name));this._availablePlugins=s.filter(i=>!a.has(i))}async _install(e){const t=this._selectedPlugin;if(!t)return;this._actionMessage=null,this._installing=!1;const s=this._displayPluginName(t),a=`install-${e}-${t}-${Date.now()}`;this.dispatchEvent(new CustomEvent("job-start",{bubbles:!0,composed:!0,detail:{id:a,label:`Adding ${s}`}}));const i=await this._streamJob(a,`${this.apiBase}/plugins/${e}/install-stream`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({names:[t],skip_verify:!0})});i!=null&&i.ok?(this._actionMessage={type:"success",text:i.message},await this._fetchAll()):this._actionMessage={type:"error",text:(i==null?void 0:i.message)||"Add failed"}}async _streamJob(e,t,s){try{const i=(await fetch(t,s)).body.getReader(),r=new TextDecoder;let o="",c=null;for(;;){const{done:p,value:b}=await i.read();if(p)break;o+=r.decode(b,{stream:!0});const u=o.split(`
`);o=u.pop();for(const d of u)if(d.startsWith("data: "))try{const m=JSON.parse(d.slice(6));m.event==="log"?this.dispatchEvent(new CustomEvent("job-log",{bubbles:!0,composed:!0,detail:{id:e,text:m.text}})):m.event==="done"&&(c={ok:m.ok,message:m.message},this.dispatchEvent(new CustomEvent("job-finish",{bubbles:!0,composed:!0,detail:{id:e,ok:m.ok,message:m.message}})))}catch{}}return c}catch(a){return this.dispatchEvent(new CustomEvent("job-finish",{bubbles:!0,composed:!0,detail:{id:e,ok:!1,message:a.message}})),{ok:!1,message:a.message}}}_displayPluginName(e){return e.split("-").map(t=>t.charAt(0).toUpperCase()+t.slice(1)).join(" ")}_switchKind(e){this.activeKind=e,this._menuOpen=!1,this.onNavigate&&this.onNavigate(e)}_displayName(){if(this.activeKind==="flow")return"Flow";if(this.activeKind==="hotkeys")return"Hotkeys";const e=S.find(t=>t.id===this.activeKind);return e?e.label:this.activeKind}render(){return n`
      <shenas-page ?loading=${this._loading} loading-text="Loading plugins..." display-name="${this._displayName()}">
        ${P(this._actionMessage)}
    ...
    `}}h(J,"properties",{_jobs:{state:!0},_collapsed:{state:!0}}),h(J,"styles",g`
    :host {
      display: block;
    }
    :host([hidden]) {
  ...
  `);customElements.define("shenas-job-panel",J);class H extends v{constructor(){super(),this.apiBase="/api",this.kind="",this.name="",this.activeTab="details",this._info=null,this._loading=!0,this._message=null,this._tables=[],this._syncing=!1,this._schemaTransforms=[],this._selectedTable=null,this._previewRows=null,this._previewLoading=!1}willUpdate(e){(e.has("kind")||e.has("name"))&&this._fetchInfo()}async _fetchInfo(){if(!this.kind||!this.name)return;this._loading=!0,this._message=null;const t=["pluginInfo(kind: $kind, name: $name)",this.kind==="dataset"?"transforms { id sourceDuckdbSchema sourceDuckdbTable targetDuckdbSchema targetDuckdbTable sourcePlugin description enabled }":""].filter(Boolean).join(" "),s=await _(this.apiBase,`query($kind: String!, $name: String!) { ${t} }`,{kind:this.kind,name:this.name});this._info=s==null?void 0:s.pluginInfo;const a=this.dbStatus,i=this.schemaPlugins,r=s==null?void 0:s.transforms,o=i?i[this.name]||[]:[];if(a){if(this.kind==="source"){const c=(a.schemas||[]).find(p=>p.name===this.name);this._tables=c?c.tables.filter(p=>!p.name.startsWith("_dlt_")):[]}else if(this.kind==="dataset"){const c=(a.schemas||[]).find(p=>p.name==="metrics");this._tables=c?c.tables.filter(p=>o.includes(p.name)):[]}}r&&(this._schemaTransforms=r.filter(c=>o.includes(c.targetDuckdbTable))),this._loading=!1,this._registerCommands()}_registerCommands(){if(!this._info)return;const e=this._info.display_name||this.name,t=[{id:`remove:${this.kind}:${this.name}`,category:"Plugin",label:`Remove ${e}`,action:()=>this._remove()}];this.kind==="dataset"&&t.unshift({id:`flush:${this.kind}:${this.name}`,category:"Plugin",label:`Flush ${e}`,action:()=>this._flush()},{id:`transform:${this.kind}:${this.name}`,category:"Plugin",label:`Transform ${e}`,action:()=>this._runTransforms()}),ee(this,`plugin-detail:${this.kind}:${this.name}`,t)}async _toggle(){var i;const e=((i=this._info)==null?void 0:i.enabled)!==!1?"disable":"enable",t=e==="enable"?"mutation($k: String!, $n: String!) { enablePlugin(kind: $k, name: $n) { ok message } }":"mutation($k: String!, $n: String!) { disablePlugin(kind: $k, name: $n) { ok message } }",{data:s}=await f(this.apiBase,t,{k:this.kind,n:this.name}),a=e==="enable"?s==null?void 0:s.enablePlugin:s==null?void 0:s.disablePlugin;this._message={type:a!=null&&a.ok?"success":"error",text:(a==null?void 0:a.message)||`${e} failed`},await this._fetchInfo(),this.dispatchEvent(new CustomEvent("plugin-state-changed",{bubbles:!0,composed:!0}))}async _sync(){this._syncing=!0,this._message=null;try{const e=await fetch(`${this.apiBase}/sync/${this.name}`,{method:"POST"});if(!e.ok){const c=await e.json().catch(()=>({}));this._message={type:"error",text:c.detail||`Sync failed (${e.status})`},this._syncing=!1;return}const t=e.body.getReader(),s=new TextDecoder;let a="",i="",r=!1;for(;;){const{done:c,value:p}=await t.read();if(c)break;const b=s.decode(p,{stream:!0});for(const u of b.split(`
`))u.startsWith("event: ")&&(a=u.slice(7).trim()),u.startsWith("data: ")&&(i=u.slice(6));a==="error"&&(r=!0)}let o="Sync complete";try{o=JSON.parse(i).message||o}catch{}this._message={type:r?"error":"success",text:o},r||await this._fetchInfo()}catch(e){this._message={type:"error",text:`Sync failed: ${e.message}`}}this._syncing=!1}async _runTransforms(){this._transforming=!0,this._message=null;try{const{data:e}=await f(this.apiBase,"mutation($s: String!) { runSchemaTransforms(schema: $s) }",{s:this.name}),t=e==null?void 0:e.runSchemaTransforms;(t==null?void 0:t.count)!=null?(this._message={type:"success",text:`Ran ${t.count} transform(s)`},await this._fetchInfo()):this._message={type:"error",text:"Transform failed"}}catch(e){this._message={type:"error",text:`Transform failed: ${e.message}`}}this._transforming=!1}async _flush(){this._message=null;try{const{data:e}=await f(this.apiBase,"mutation($s: String!) { flushSchema(schemaPlugin: $s) }",{s:this.name}),t=e==null?void 0:e.flushSchema;(t==null?void 0:t.rows_deleted)!=null?(this._message={type:"success",text:`Flushed ${t.rows_deleted} rows`},await this._fetchInfo()):this._message={type:"error",text:"Flush failed"}}catch(e){this._message={type:"error",text:`Flush failed: ${e.message}`}}}async _remove(){var s;const e=((s=this._pluginInfo)==null?void 0:s.display_name)||this.name.replace("-"," ").replace(/\b\w/g,a=>a.toUpperCase()),t=`remove-${this.kind}-${this.name}-${Date.now()}`;this.dispatchEvent(new CustomEvent("job-start",{bubbles:!0,composed:!0,detail:{id:t,label:`Removing ${e}`}}));try{const i=(await fetch(`${this.apiBase}/plugins/${this.kind}/${this.name}/remove-stream`,{method:"POST"})).body.getReader(),r=new TextDecoder;let o="",c=!1;for(;;){const{done:p,value:b}=await i.read();if(p)break;o+=r.decode(b,{stream:!0});const u=o.split(`
`);o=u.pop();for(const d of u)if(d.startsWith("data: "))try{const m=JSON.parse(d.slice(6));m.event==="log"?this.dispatchEvent(new CustomEvent("job-log",{bubbles:!0,composed:!0,detail:{id:t,text:m.text}})):m.event==="done"&&(c=m.ok,this.dispatchEvent(new CustomEvent("job-finish",{bubbles:!0,composed:!0,detail:{id:t,ok:m.ok,message:m.message}})))}catch{}}c?(window.history.pushState({},"",`/settings/${this.kind}`),window.dispatchEvent(new PopStateEvent("popstate"))):this._message={type:"error",text:"Remove failed"}}catch(a){this.dispatchEvent(new CustomEvent("job-finish",{bubbles:!0,composed:!0,detail:{id:t,ok:!1,message:a.message}})),this._message={type:"error",text:a.message}}}_switchTab(e){this.activeTab=e;const t=`/settings/${this.kind}/${this.name}`,s=e==="details"?t:`${t}/${e}`;window.history.pushState({},"",s)}async _fetchPreview(e){if(this._selectedTable=e,!e){this._previewRows=null;return}this._previewLoading=!0;const t=this.kind==="dataset"?"metrics":this.name;this._previewRows=await E(this.apiBase,`SELECT * FROM "${t}"."${e}" ORDER BY 1 DESC LIMIT 100`),this._previewLoading=!1}_renderData(){var t,s;const e=this._tables||[];if(e.length===0)return n`<p style="color:var(--shenas-text-muted,#888)">No tables synced yet.</p>`;if(!this._selectedTable){const a=(t=this._info)==null?void 0:t.primary_table,i=a&&e.some(r=>r.name===a)?a:(s=e[0])==null?void 0:s.name;i&&this._fetchPreview(i)}return n`
    ...
    `:""}}h(H,"properties",{apiBase:{type:String,attribute:"api-base"},kind:{type:String},name:{type:String},activeTab:{type:String,attribute:"active-tab"},dbStatus:{type:Object},schemaPlugins:{type:Object},_info:{state:!0},_loading:{state:!0},_message:{state:!0},_tables:{state:!0},_syncing:{state:!0},_transforming:{state:!0},_schemaTransforms:{state:!0},_selectedTable:{state:!0},_previewRows:{state:!0},_previewLoading:{state:!0}}),h(H,"styles",[x,N,C,ne,g`
      :host {
        display: block;
      }
      .back {
    ...
    `}}h(V,"properties",{apiBase:{type:String,attribute:"api-base"},source:{type:String},_transforms:{state:!0},_loading:{state:!0},_editing:{state:!0},_editSql:{state:!0},_message:{state:!0},_previewRows:{state:!0},_creating:{state:!0},_newForm:{state:!0},_dbTables:{state:!0},_schemaTables:{state:!0}}),h(V,"styles",[Q,x,I,C,g`
      :host {
        display: block;
      }
      .edit-panel {
