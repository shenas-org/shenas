...
`;class I extends g{constructor(){super(),this.loading=!1,this.empty=!1,this.loadingText="Loading...",this.emptyText="No data",this.displayName=""}updated(e){e.has("displayName")&&this.displayName&&this.dispatchEvent(new CustomEvent("page-title",{bubbles:!0,composed:!0,detail:{title:this.displayName}}))}render(){return this.loading?n`<p class="loading">${this.loadingText}</p>`:this.empty?n`<p class="empty">${this.emptyText}</p>`:n`<slot></slot>`}}d(I,"properties",{loading:{type:Boolean,reflect:!0},empty:{type:Boolean,reflect:!0},loadingText:{type:String,attribute:"loading-text"},emptyText:{type:String,attribute:"empty-text"},displayName:{type:String,attribute:"display-name"}}),d(I,"styles",[C,m`
    ...
    `]);customElements.define("shenas-page",I);class O extends g{constructor(){super(),this.enabled=!1,this.toggleable=!1}updated(){this.title=this.enabled?"Enabled":"Disabled"}render(){return n`<div class="track" @click=${this._onClick}><div class="knob"></div></div>`}_onClick(){this.toggleable&&this.dispatchEvent(new CustomEvent("toggle",{bubbles:!0,composed:!0}))}}d(O,"properties",{enabled:{type:Boolean,reflect:!0},toggleable:{type:Boolean,reflect:!0}}),d(O,"styles",m`
    :host {
  ...
  `);customElements.define("status-toggle",O);class N extends g{constructor(){super(),this.columns=[],this.rows=[],this.rowClass=null,this.actions=null,this.emptyText="No items",this.showAdd=!1}_onAdd(){this.dispatchEvent(new CustomEvent("add",{bubbles:!0,composed:!0}))}render(){const e=typeof this.actions=="function",t=this.showAdd?n`<div class="add-row"><button class="add-btn" title="Add" @click=${this._onAdd}>+</button></div>`:"";return!this.rows||this.rows.length===0?n`<p class="empty">${this.emptyText}</p>${t}`:n`
      ...
      ${t}
    `}}d(N,"properties",{columns:{type:Array},rows:{type:Array},rowClass:{type:Object},actions:{type:Object},emptyText:{type:String,attribute:"empty-text"},showAdd:{type:Boolean,attribute:"show-add"}}),d(N,"styles",[Z,x,C,m`
      :host {
        display: block;
      }
      .mono {
    ...
    `]);customElements.define("shenas-data-list",N);class P extends g{constructor(){super(),this.title="",this.submitLabel="Save"}render(){return n`
    ...
    `}}d(L,"properties",{apiBase:{type:String,attribute:"api-base"},_loading:{state:!0},_empty:{state:!0}}),d(L,"styles",[C,m`
      :host {
        display: block;
      }
      #cy {
    ...
    `}}d(A,"properties",{apiBase:{type:String,attribute:"api-base"},pipeName:{type:String,attribute:"pipe-name"},_fields:{state:!0},_instructions:{state:!0},_loading:{state:!0},_message:{state:!0},_needsMfa:{state:!0},_oauthUrl:{state:!0},_submitting:{state:!0},_stored:{state:!0}}),d(A,"styles",[x,B,E,m`
      :host {
        display: block;
      }
      .instructions {
    ...
    `]);customElements.define("shenas-auth",A);const f=class f extends g{constructor(){super(),this.apiBase="/api",this.kind="",this.name="",this._config=null,this._loading=!0,this._message=null,this._editing=null,this._editValue="",this._freqNum="",this._freqUnit="hours"}willUpdate(e){(e.has("kind")||e.has("name"))&&this._fetchConfig()}async _fetchConfig(){if(!this.kind||!this.name)return;this._loading=!0;const e=await h(this.apiBase,`/config?kind=${this.kind}&name=${this.name}`);this._config=e&&e.length>0?e[0]:null,this._loading=!1}_startEdit(e,t){if(this._editing=e,this._editValue=t||"",f._DURATION_FIELDS.has(e)&&t){const s=parseFloat(t);s>=1440&&s%1440===0?(this._freqNum=String(s/1440),this._freqUnit="days"):s>=60&&s%60===0?(this._freqNum=String(s/60),this._freqUnit="hours"):s>=1?(this._freqNum=String(s),this._freqUnit="minutes"):(this._freqNum=String(s*60),this._freqUnit="seconds")}else f._DURATION_FIELDS.has(e)&&(this._freqNum="",this._freqUnit="hours")}_cancelEdit(){this._editing=null,this._editValue=""}_freqToMinutes(){const e=parseFloat(this._freqNum);return isNaN(e)||e<=0?null:String(Math.round(e*f._UNIT_MULTIPLIERS[this._freqUnit]))}_formatFreq(e){const t=parseFloat(e);return isNaN(t)?e:t>=1440&&t%1440===0?`${t/1440} day${t/1440!==1?"s":""}`:t>=60&&t%60===0?`${t/60} hour${t/60!==1?"s":""}`:t>=1?`${t} minute${t!==1?"s":""}`:`${t*60} second${t*60!==1?"s":""}`}async _saveEdit(e){const t=f._DURATION_FIELDS.has(e)?this._freqToMinutes():this._editValue;if(f._DURATION_FIELDS.has(e)&&t===null){this._message={type:"error",text:"Enter a positive number"};return}const{ok:s,data:a}=await b(this.apiBase,`/config/${this.kind}/${this.name}`,{method:"PUT",json:{key:e,value:t}});s?(this._message={type:"success",text:`Updated ${e}`},this._editing=null,await this._fetchConfig()):this._message={type:"error",text:(a==null?void 0:a.detail)||"Update failed"}}render(){var t;const e=!this._config||this._config.entries.length===0;return n`
      <shenas-page ?loading=${this._loading} ?empty=${e}
    ...
    `}_renderFreqEdit(e){return n`
      <div class="edit-row">
        ...
        <select @change=${t=>{this._freqUnit=t.target.value}}>
          ${Object.keys(f._UNIT_MULTIPLIERS).map(t=>n`
      ...
      </div>`}_renderEntry(e){const t=this._editing===e.key,s=f._DURATION_FIELDS.has(e.key),a=s&&e.value?this._formatFreq(e.value):e.value;return n`
    ...
    `}};d(f,"properties",{apiBase:{type:String,attribute:"api-base"},kind:{type:String},name:{type:String},_config:{state:!0},_loading:{state:!0},_message:{state:!0},_editing:{state:!0},_editValue:{state:!0},_freqNum:{state:!0},_freqUnit:{state:!0}}),d(f,"styles",[x,B,E,m`
      :host {
        display: block;
      }
      .config-row {
    ...
    `]);customElements.define("shenas-settings",M);class j extends g{constructor(){super(),this.apiBase="/api",this.kind="",this.name="",this.activeTab="details",this._info=null,this._loading=!0,this._message=null,this._tables=[],this._syncing=!1,this._schemaTransforms=[],this._selectedTable=null,this._previewRows=null,this._previewLoading=!1}willUpdate(e){(e.has("kind")||e.has("name"))&&this._fetchInfo()}async _fetchInfo(){if(!this.kind||!this.name)return;this._loading=!0,this._message=null,this._info=await h(this.apiBase,`/plugins/${this.kind}/${this.name}/info`);const e=this.kind==="pipe"||this.kind==="schema",[t,s,a]=await Promise.all([e?h(this.apiBase,"/db/status"):null,this.kind==="schema"?h(this.apiBase,"/db/schema-plugins"):null,this.kind==="schema"?h(this.apiBase,"/transforms"):null]),i=s?s[this.name]||[]:[];if(t){if(this.kind==="pipe"){const o=(t.schemas||[]).find(r=>r.name===this.name);this._tables=o?o.tables.filter(r=>!r.name.startsWith("_dlt_")):[]}else if(this.kind==="schema"){const o=(t.schemas||[]).find(r=>r.name==="metrics");this._tables=o?o.tables.filter(r=>i.includes(r.name)):[]}}a&&(this._schemaTransforms=a.filter(o=>i.includes(o.target_duckdb_table))),this._loading=!1,this._registerCommands()}_registerCommands(){if(!this._info)return;const e=this._info.display_name||this.name,t=[{id:`remove:${this.kind}:${this.name}`,category:"Plugin",label:`Remove ${e}`,action:()=>this._remove()}];this.kind==="schema"&&t.unshift({id:`flush:${this.kind}:${this.name}`,category:"Plugin",label:`Flush ${e}`,action:()=>this._flush()},{id:`transform:${this.kind}:${this.name}`,category:"Plugin",label:`Transform ${e}`,action:()=>this._runTransforms()}),X(this,`plugin-detail:${this.kind}:${this.name}`,t)}async _toggle(){var s;const e=((s=this._info)==null?void 0:s.enabled)!==!1?"disable":"enable",{data:t}=await b(this.apiBase,`/plugins/${this.kind}/${this.name}/${e}`,{method:"POST"});this._message={type:t!=null&&t.ok?"success":"error",text:(t==null?void 0:t.message)||`${e} failed`},await this._fetchInfo(),this.dispatchEvent(new CustomEvent("plugin-state-changed",{bubbles:!0,composed:!0}))}async _sync(){this._syncing=!0,this._message=null;try{const e=await fetch(`${this.apiBase}/sync/${this.name}`,{method:"POST"});if(!e.ok){const p=await e.json().catch(()=>({}));this._message={type:"error",text:p.detail||`Sync failed (${e.status})`},this._syncing=!1;return}const t=e.body.getReader(),s=new TextDecoder;let a="",i="",o=!1;for(;;){const{done:p,value:_}=await t.read();if(p)break;const w=s.decode(_,{stream:!0});for(const v of w.split(`
`))v.startsWith("event: ")&&(a=v.slice(7).trim()),v.startsWith("data: ")&&(i=v.slice(6));a==="error"&&(o=!0)}let r="Sync complete";try{r=JSON.parse(i).message||r}catch{}this._message={type:o?"error":"success",text:r},o||await this._fetchInfo()}catch(e){this._message={type:"error",text:`Sync failed: ${e.message}`}}this._syncing=!1}async _runTransforms(){this._transforming=!0,this._message=null;try{const{data:e}=await b(this.apiBase,`/transforms/run/schema/${this.name}`,{method:"POST"});(e==null?void 0:e.count)!=null?(this._message={type:"success",text:`Ran ${e.count} transform(s)`},await this._fetchInfo()):this._message={type:"error",text:(e==null?void 0:e.detail)||"Transform failed"}}catch(e){this._message={type:"error",text:`Transform failed: ${e.message}`}}this._transforming=!1}async _flush(){this._message=null;try{const{data:e}=await b(this.apiBase,`/db/schema/${this.name}/flush`,{method:"DELETE"});(e==null?void 0:e.rows_deleted)!=null?(this._message={type:"success",text:`Flushed ${e.rows_deleted} rows`},await this._fetchInfo()):this._message={type:"error",text:(e==null?void 0:e.detail)||"Flush failed"}}catch(e){this._message={type:"error",text:`Flush failed: ${e.message}`}}}async _remove(){const{data:e}=await b(this.apiBase,`/plugins/${this.kind}/${this.name}`,{method:"DELETE"});e!=null&&e.ok?(window.history.pushState({},"",`/settings/${this.kind}`),window.dispatchEvent(new PopStateEvent("popstate"))):this._message={type:"error",text:e.message||"Remove failed"}}_switchTab(e){this.activeTab=e;const t=`/settings/${this.kind}/${this.name}`,s=e==="details"?t:`${t}/${e}`;window.history.pushState({},"",s)}async _fetchPreview(e){if(this._selectedTable=e,!e){this._previewRows=null;return}this._previewLoading=!0;const t=this.kind==="schema"?"metrics":this.name;this._previewRows=await h(this.apiBase,`/db/preview/${t}/${e}?limit=100`),this._previewLoading=!1}_renderData(){var t;const e=this._tables||[];if(e.length===0)return n`<p style="color:var(--shenas-text-muted,#888)">No tables synced yet.</p>`;if(!this._selectedTable&&((t=this._info)!=null&&t.primary_table)){const s=this._info.primary_table;e.some(a=>a.name===s)&&this._fetchPreview(s)}return n`
    ...
    `:""}}d(j,"properties",{apiBase:{type:String,attribute:"api-base"},kind:{type:String},name:{type:String},activeTab:{type:String,attribute:"active-tab"},_info:{state:!0},_loading:{state:!0},_message:{state:!0},_tables:{state:!0},_syncing:{state:!0},_transforming:{state:!0},_schemaTransforms:{state:!0},_selectedTable:{state:!0},_previewRows:{state:!0},_previewLoading:{state:!0}}),d(j,"styles",[x,H,E,se,m`
      :host {
        display: block;
      }
      .back {
