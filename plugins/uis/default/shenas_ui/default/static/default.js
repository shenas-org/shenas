...
`);o=u.pop();for(const d of u)if(d.startsWith("data: "))try{const m=JSON.parse(d.slice(6));m.event==="log"?this.dispatchEvent(new CustomEvent("job-log",{bubbles:!0,composed:!0,detail:{id:e,text:m.text}})):m.event==="done"&&(c={ok:m.ok,message:m.message},this.dispatchEvent(new CustomEvent("job-finish",{bubbles:!0,composed:!0,detail:{id:e,ok:m.ok,message:m.message}})))}catch{}}return c}catch(a){return this.dispatchEvent(new CustomEvent("job-finish",{bubbles:!0,composed:!0,detail:{id:e,ok:!1,message:a.message}})),{ok:!1,message:a.message}}}_displayPluginName(e){return e.split("-").map(t=>t.charAt(0).toUpperCase()+t.slice(1)).join(" ")}_switchKind(e){this.activeKind=e,this._menuOpen=!1,this.onNavigate&&this.onNavigate(e)}_displayName(){if(this.activeKind==="data-flow")return"Data Flow";if(this.activeKind==="hotkeys")return"Hotkeys";const e=S.find(t=>t.id===this.activeKind);return e?e.label:this.activeKind}render(){return n`
    ...
    `}_formatFreq(e){return e>=1440&&e%1440===0?`${e/1440}d`:e>=60&&e%60===0?`${e/60}h`:e>=1?`${e}m`:`${e*60}s`}_renderKind(e){var a;const t=this._plugins[e]||[],s=((a=S.find(i=>i.id===e))==null?void 0:a.label)||e;return n`
      <h3>${s}</h3>
      <shenas-data-list
        .columns=${[{label:"Name",render:i=>n`<a href="/settings/${e}/${i.name}">${i.displayName||i.name}</a>`},...e==="pipe"?[{label:"Last Synced",class:"mono",render:i=>i.syncedAt?i.syncedAt.slice(0,16).replace("T"," "):"never"}]:[],{label:"Status",render:i=>e==="pipe"&&i.hasAuth===!1?n`<span style="color:var(--shenas-error,#c62828);font-size:0.8rem">Needs Auth</span>`:n`<status-toggle ?enabled=${i.enabled!==!1} toggleable @toggle=${()=>this._togglePlugin(e,i.name,i.enabled!==!1)}></status-toggle>`}]}
        .rows=${t}
  ...
  `);customElements.define("shenas-job-panel",J);class H extends v{constructor(){super(),this.apiBase="/api",this.kind="",this.name="",this.activeTab="details",this._info=null,this._loading=!0,this._message=null,this._tables=[],this._syncing=!1,this._schemaTransforms=[],this._selectedTable=null,this._previewRows=null,this._previewLoading=!1}willUpdate(e){(e.has("kind")||e.has("name"))&&this._fetchInfo()}async _fetchInfo(){if(!this.kind||!this.name)return;this._loading=!0,this._message=null;const t=["pluginInfo(kind: $kind, name: $name)",this.kind==="schema"?"transforms { id sourceDuckdbSchema sourceDuckdbTable targetDuckdbSchema targetDuckdbTable sourcePlugin description enabled }":""].filter(Boolean).join(" "),s=await _(this.apiBase,`query($kind: String!, $name: String!) { ${t} }`,{kind:this.kind,name:this.name});this._info=s==null?void 0:s.pluginInfo;const a=this.dbStatus,i=this.schemaPlugins,r=s==null?void 0:s.transforms,o=i?i[this.name]||[]:[];if(a){if(this.kind==="pipe"){const c=(a.schemas||[]).find(p=>p.name===this.name);this._tables=c?c.tables.filter(p=>!p.name.startsWith("_dlt_")):[]}else if(this.kind==="schema"){const c=(a.schemas||[]).find(p=>p.name==="metrics");this._tables=c?c.tables.filter(p=>o.includes(p.name)):[]}}r&&(this._schemaTransforms=r.filter(c=>o.includes(c.targetDuckdbTable))),this._loading=!1,this._registerCommands()}_registerCommands(){if(!this._info)return;const e=this._info.display_name||this.name,t=[{id:`remove:${this.kind}:${this.name}`,category:"Plugin",label:`Remove ${e}`,action:()=>this._remove()}];this.kind==="schema"&&t.unshift({id:`flush:${this.kind}:${this.name}`,category:"Plugin",label:`Flush ${e}`,action:()=>this._flush()},{id:`transform:${this.kind}:${this.name}`,category:"Plugin",label:`Transform ${e}`,action:()=>this._runTransforms()}),ee(this,`plugin-detail:${this.kind}:${this.name}`,t)}async _toggle(){var i;const e=((i=this._info)==null?void 0:i.enabled)!==!1?"disable":"enable",t=e==="enable"?"mutation($k: String!, $n: String!) { enablePlugin(kind: $k, name: $n) { ok message } }":"mutation($k: String!, $n: String!) { disablePlugin(kind: $k, name: $n) { ok message } }",{data:s}=await f(this.apiBase,t,{k:this.kind,n:this.name}),a=e==="enable"?s==null?void 0:s.enablePlugin:s==null?void 0:s.disablePlugin;this._message={type:a!=null&&a.ok?"success":"error",text:(a==null?void 0:a.message)||`${e} failed`},await this._fetchInfo(),this.dispatchEvent(new CustomEvent("plugin-state-changed",{bubbles:!0,composed:!0}))}async _sync(){this._syncing=!0,this._message=null;try{const e=await fetch(`${this.apiBase}/sync/${this.name}`,{method:"POST"});if(!e.ok){const c=await e.json().catch(()=>({}));this._message={type:"error",text:c.detail||`Sync failed (${e.status})`},this._syncing=!1;return}const t=e.body.getReader(),s=new TextDecoder;let a="",i="",r=!1;for(;;){const{done:c,value:p}=await t.read();if(c)break;const b=s.decode(p,{stream:!0});for(const u of b.split(`
