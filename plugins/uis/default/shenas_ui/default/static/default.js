var Q=Object.defineProperty;var Y=(l,e,t)=>e in l?Q(l,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):l[e]=t;var d=(l,e,t)=>Y(l,typeof e!="symbol"?e+"":e,t);import{LitElement as g,css as p,html as n}from"lit";import V,{dagre as ee}from"cytoscape";import{Router as te}from"@lit-labs/router";class D extends g{constructor(){super(),this.open=!1,this.commands=[],this._query="",this._filtered=[],this._selectedIndex=0}updated(e){e.has("open")&&this.open&&(this._query="",this._selectedIndex=0,this._filter(),requestAnimationFrame(()=>{const t=this.renderRoot.querySelector("input");t&&t.focus()})),e.has("commands")&&this._filter()}_filter(){const e=this._query.toLowerCase();e?this._filtered=this.commands.filter(t=>t.label.toLowerCase().includes(e)||t.category.toLowerCase().includes(e)||(t.description||"").toLowerCase().includes(e)):this._filtered=this.commands,this._selectedIndex>=this._filtered.length&&(this._selectedIndex=Math.max(0,this._filtered.length-1))}_onInput(e){this._query=e.target.value,this._selectedIndex=0,this._filter()}_onKeydown(e){if(e.key==="ArrowDown")e.preventDefault(),this._filtered.length>0&&(this._selectedIndex=Math.min(this._selectedIndex+1,this._filtered.length-1)),this._scrollToSelected();else if(e.key==="ArrowUp")e.preventDefault(),this._selectedIndex=Math.max(this._selectedIndex-1,0),this._scrollToSelected();else if(e.key==="Enter"){e.preventDefault();const t=this._filtered[this._selectedIndex];t&&this._execute(t)}else e.key==="Escape"&&this._close()}_scrollToSelected(){requestAnimationFrame(()=>{const e=this.renderRoot.querySelector(".item.selected");e&&e.scrollIntoView({block:"nearest"})})}_execute(e){this.dispatchEvent(new CustomEvent("execute",{detail:e,bubbles:!0,composed:!0}))}_close(){this.dispatchEvent(new CustomEvent("close",{bubbles:!0,composed:!0}))}render(){return this.open?n`
    ...
    `:n``}}d(D,"properties",{open:{type:Boolean,reflect:!0},commands:{type:Array},_query:{state:!0},_filtered:{state:!0},_selectedIndex:{state:!0}}),d(D,"styles",p`
    :host {
      display: none;
    }
    :host([open]) {
  ...
  `);customElements.define("shenas-command-palette",D);const X=p`
  table {
    width: 100%;
...
`;class I extends g{constructor(){super(),this.loading=!1,this.empty=!1,this.loadingText="Loading...",this.emptyText="No data",this.displayName=""}updated(e){e.has("displayName")&&this.displayName&&this.dispatchEvent(new CustomEvent("page-title",{bubbles:!0,composed:!0,detail:{title:this.displayName}}))}render(){return this.loading?n`<p class="loading">${this.loadingText}</p>`:this.empty?n`<p class="empty">${this.emptyText}</p>`:n`<slot></slot>`}}d(I,"properties",{loading:{type:Boolean,reflect:!0},empty:{type:Boolean,reflect:!0},loadingText:{type:String,attribute:"loading-text"},emptyText:{type:String,attribute:"empty-text"},displayName:{type:String,attribute:"display-name"}}),d(I,"styles",[C]);customElements.define("shenas-page",I);class N extends g{constructor(){super(),this.enabled=!1,this.toggleable=!1}updated(){this.title=this.enabled?"Enabled":"Disabled"}render(){return n`<div class="track" @click=${this._onClick}><div class="knob"></div></div>`}_onClick(){this.toggleable&&this.dispatchEvent(new CustomEvent("toggle",{bubbles:!0,composed:!0}))}}d(N,"properties",{enabled:{type:Boolean,reflect:!0},toggleable:{type:Boolean,reflect:!0}}),d(N,"styles",p`
    :host {
  ...
  `);customElements.define("status-toggle",N);class O extends g{constructor(){super(),this.columns=[],this.rows=[],this.rowClass=null,this.actions=null,this.emptyText="No items",this.showAdd=!1}_onAdd(){this.dispatchEvent(new CustomEvent("add",{bubbles:!0,composed:!0}))}render(){const e=typeof this.actions=="function",t=this.showAdd?n`<div class="add-row"><button class="add-btn" title="Add" @click=${this._onAdd}>+</button></div>`:"";return!this.rows||this.rows.length===0?n`<p class="empty">${this.emptyText}</p>${t}`:n`
      ...
      ${t}
    `}}d(O,"properties",{columns:{type:Array},rows:{type:Array},rowClass:{type:Object},actions:{type:Object},emptyText:{type:String,attribute:"empty-text"},showAdd:{type:Boolean,attribute:"show-add"}}),d(O,"styles",[X,x,C,p`
      :host {
        display: block;
      }
      .mono {
    ...
    `]);customElements.define("shenas-data-list",O);class A extends g{constructor(){super(),this.title="",this.submitLabel="Save"}render(){return n`
    ...
    `}_onSubmit(){this.dispatchEvent(new CustomEvent("submit",{bubbles:!0,composed:!0}))}_onCancel(){this.dispatchEvent(new CustomEvent("cancel",{bubbles:!0,composed:!0}))}}d(A,"properties",{title:{type:String},submitLabel:{type:String,attribute:"submit-label"}}),d(A,"styles",[x,p`
      :host {
        display: block;
        margin: 1rem 0;
        padding: 1rem;
    ...
    `]);customElements.define("shenas-form-panel",A);async function h(l,e,t={}){const{method:s="GET",json:a,...i}=t,r={method:s,...i};a!==void 0&&(r.headers={"Content-Type":"application/json",...i.headers},r.body=JSON.stringify(a));const o=await fetch(`${l}${e}`,r);return o.ok?o.json():(console.warn(`apiFetch ${s} ${e} failed: ${o.status}`),null)}async function v(l,e,t={}){const{method:s="GET",json:a,...i}=t,r={method:s,...i};a!==void 0&&(r.headers={"Content-Type":"application/json",...i.headers},r.body=JSON.stringify(a));const o=await fetch(`${l}${e}`,r),m=await o.json().catch(()=>null);return{ok:o.ok,status:o.status,data:m}}function z(l){return l?n`<div class="message ${l.type}">${l.text}</div>`:""}function Z(l,e,t){l.dispatchEvent(new CustomEvent("register-command",{bubbles:!0,composed:!0,detail:{componentId:e,commands:t}}))}let G=!1;class L extends g{constructor(){super(),this.apiBase="/api",this._loading=!0,this._empty=!1,this._cy=null,this._elements=null,this._resizeObserver=null}connectedCallback(){super.connectedCallback(),this._fetchData()}disconnectedCallback(){super.disconnectedCallback(),this._cy&&(this._cy.destroy(),this._cy=null),this._resizeObserver&&(this._resizeObserver.disconnect(),this._resizeObserver=null)}async _fetchData(){this._loading=!0;try{const[e,t,s,a,i,r]=await Promise.all([h(this.apiBase,"/plugins/pipe"),h(this.apiBase,"/plugins/schema"),h(this.apiBase,"/transforms"),h(this.apiBase,"/db/schema-plugins"),h(this.apiBase,"/plugins/component"),h(this.apiBase,"/dependencies")]);this._buildElements(e||[],t||[],s||[],a||{},i||[],r||{})}catch(e){console.error("Failed to fetch overview data:",e)}this._loading=!1}_buildElements(e,t,s,a,i,r){const o=[],m=new Set,y={};for(const[c,u]of Object.entries(a))for(const k of u)y[k]=c;for(const c of e){const u=`pipe:${c.name}`;m.add(u),o.push({data:{id:u,label:c.display_name||c.name,kind:"pipe",enabled:c.enabled!==!1?"yes":"no"}})}for(const c of t){const u=`schema:${c.name}`;m.add(u),o.push({data:{id:u,label:c.display_name||c.name,kind:"schema",enabled:c.enabled!==!1?"yes":"no"}})}for(const c of i){const u=`component:${c.name}`;m.add(u),o.push({data:{id:u,label:c.display_name||c.name,kind:"component",enabled:c.enabled!==!1?"yes":"no"}})}for(const c of s){const u=`pipe:${c.source_plugin}`,k=y[c.target_duckdb_table],S=k?`schema:${k}`:null;if(!S||!m.has(u)||!m.has(S))continue;const _=c.description||`${c.source_duckdb_table} -> ${c.target_duckdb_table}`,w=_.length>30?_.slice(0,28)+"...":_;o.push({data:{id:`transform:${c.id}`,source:u,target:S,label:w,enabled:c.enabled?"yes":"no",sourcePlugin:c.source_plugin,edgeType:"transform"}})}const T=new Set;for(const c of o)c.data.edgeType==="transform"&&T.add(`${c.data.source}:${c.data.target}`);const f=new Set;for(const[c,u]of Object.entries(r))for(const k of u){const S=c.split(":")[0];let _,w;if(S==="component"?(_=k,w=c):(_=c,w=k),!m.has(_)||!m.has(w)||S==="pipe"&&T.has(`${_}:${w}`))continue;const R=`dep:${_}:${w}`;f.has(R)||(f.add(R),o.push({data:{id:R,source:_,target:w,edgeType:"dependency"}}))}this._elements=o,this._empty=o.filter(c=>c.data.source).length===0}_initCytoscape(){const e=this.renderRoot.querySelector("#cy");!e||!this._elements||(G||(V.use(ee),G=!0),this._cy&&this._cy.destroy(),this._cy=V({container:e,elements:this._elements,style:[{selector:"node",style:{label:"data(label)","text-valign":"center","text-halign":"center","font-size":12,color:"#fff","text-wrap":"wrap","text-max-width":100,width:120,height:40,shape:"round-rectangle"}},{selector:'node[kind="pipe"]',style:{"background-color":"#4a90d9",cursor:"pointer"}},{selector:'node[kind="schema"]',style:{"background-color":"#66bb6a",cursor:"pointer"}},{selector:'node[kind="component"]',style:{"background-color":"#ffa726",cursor:"pointer"}},{selector:'node[enabled="no"]',style:{opacity:.4,"border-width":2,"border-color":"#999","border-style":"dashed"}},{selector:"edge",style:{"curve-style":"bezier","target-arrow-shape":"triangle","target-arrow-color":"#999","line-color":"#999",cursor:"pointer",width:2,label:"data(label)","font-size":9,color:"#888","text-rotation":"autorotate","text-margin-y":-8}},{selector:'edge[enabled="yes"]',style:{"line-style":"solid"}},{selector:'edge[enabled="no"]',style:{"line-style":"dashed","line-color":"#ccc","target-arrow-color":"#ccc",opacity:.5}},{selector:'edge[edgeType="dependency"]',style:{"line-style":"dotted","line-color":"#bbb","target-arrow-color":"#bbb",width:1.5,label:""}}],layout:{name:"dagre",rankDir:"LR",nodeSep:60,rankSep:150,padding:30},userZoomingEnabled:!0,userPanningEnabled:!0,boxSelectionEnabled:!1}),this._cy.on("tap","node",t=>{const s=t.target.data(),a=s.id.substring(s.id.indexOf(":")+1);let i;if(s.kind==="pipe")i=`/settings/pipe/${a}`;else if(s.kind==="schema")i=`/settings/schema/${a}`;else if(s.kind==="component")i=`/settings/component/${a}`;else return;this.dispatchEvent(new CustomEvent("navigate",{bubbles:!0,composed:!0,detail:{path:i}}))}),this._cy.on("tap","edge",t=>{const s=t.target.data("sourcePlugin");s&&this.dispatchEvent(new CustomEvent("navigate",{bubbles:!0,composed:!0,detail:{path:`/settings/pipe/${s}`}}))}),this._resizeObserver&&this._resizeObserver.disconnect(),this._resizeObserver=new ResizeObserver(()=>{this._cy&&(this._cy.resize(),this._cy.fit(void 0,30))}),this._resizeObserver.observe(e))}firstUpdated(){!this._loading&&this._elements&&this._initCytoscape()}updated(e){e.has("_loading")&&!this._loading&&this._elements&&requestAnimationFrame(()=>this._initCytoscape())}render(){return n`
    ...
    `}}d(L,"properties",{apiBase:{type:String,attribute:"api-base"},_loading:{state:!0},_empty:{state:!0}}),d(L,"styles",[C,p`
      :host {
        display: block;
      }
      #cy {
    ...
    `}}d(P,"properties",{apiBase:{type:String,attribute:"api-base"},pipeName:{type:String,attribute:"pipe-name"},_fields:{state:!0},_instructions:{state:!0},_loading:{state:!0},_message:{state:!0},_needsMfa:{state:!0},_oauthUrl:{state:!0},_submitting:{state:!0},_stored:{state:!0}}),d(P,"styles",[x,B,E,p`
      :host {
        display: block;
      }
      .instructions {
    ...
    `}};d(b,"properties",{apiBase:{type:String,attribute:"api-base"},kind:{type:String},name:{type:String},_config:{state:!0},_loading:{state:!0},_message:{state:!0},_editing:{state:!0},_editValue:{state:!0},_freqNum:{state:!0},_freqUnit:{state:!0}}),d(b,"styles",[x,B,E,p`
      :host {
        display: block;
      }
      .config-row {
    ...
    `}}d(F,"properties",{apiBase:{type:String,attribute:"api-base"},actions:{type:Array},_bindings:{state:!0},_recording:{state:!0},_recordedKey:{state:!0},_conflict:{state:!0},_loading:{state:!0},_filter:{state:!0}}),d(F,"styles",[x,E,C,p`
      :host {
        display: block;
      }
      .toolbar {
    ...
    `}_formatTime(e){return e?String(e).replace("T"," ").slice(0,23):"-"}}d(K,"properties",{apiBase:{type:String,attribute:"api-base"},pipe:{type:String},_activeTab:{state:!0},_logs:{state:!0},_spans:{state:!0},_loading:{state:!0},_search:{state:!0},_severity:{state:!0},_expanded:{state:!0},_live:{state:!0}}),d(K,"styles",[x,C,p`
      :host {
        display: flex;
        flex-direction: column;
        height: 100%;
    ...
    `}_formatFreq(e){return e>=1440&&e%1440===0?`${e/1440}d`:e>=60&&e%60===0?`${e/60}h`:e>=1?`${e}m`:`${e*60}s`}_renderKind(e){var a;const t=this._plugins[e]||[],s=((a=$.find(i=>i.id===e))==null?void 0:a.label)||e;return n`
      <h3>${s}</h3>
      <shenas-data-list
    ...
    `}}d(j,"properties",{apiBase:{type:String,attribute:"api-base"},activeKind:{type:String,attribute:"active-kind"},onNavigate:{type:Function},allActions:{type:Array},_plugins:{state:!0},_loading:{state:!0},_actionMessage:{state:!0},_installing:{state:!0},_menuOpen:{state:!0}}),d(j,"styles",[x,B,H,E,p`
      :host {
        display: block;
        height: 100%;
      }
    ...
    `]);customElements.define("shenas-settings",j);class M extends g{constructor(){super(),this.apiBase="/api",this.kind="",this.name="",this.activeTab="details",this._info=null,this._loading=!0,this._message=null,this._tables=[],this._syncing=!1,this._schemaTransforms=[],this._selectedTable=null,this._previewRows=null,this._previewLoading=!1}willUpdate(e){(e.has("kind")||e.has("name"))&&this._fetchInfo()}async _fetchInfo(){if(!this.kind||!this.name)return;this._loading=!0,this._message=null,this._info=await h(this.apiBase,`/plugins/${this.kind}/${this.name}/info`);const e=this.kind==="pipe"||this.kind==="schema",[t,s,a]=await Promise.all([e?h(this.apiBase,"/db/status"):null,this.kind==="schema"?h(this.apiBase,"/db/schema-plugins"):null,this.kind==="schema"?h(this.apiBase,"/transforms"):null]),i=s?s[this.name]||[]:[];if(t){if(this.kind==="pipe"){const r=(t.schemas||[]).find(o=>o.name===this.name);this._tables=r?r.tables.filter(o=>!o.name.startsWith("_dlt_")):[]}else if(this.kind==="schema"){const r=(t.schemas||[]).find(o=>o.name==="metrics");this._tables=r?r.tables.filter(o=>i.includes(o.name)):[]}}a&&(this._schemaTransforms=a.filter(r=>i.includes(r.target_duckdb_table))),this._loading=!1,this._registerCommands()}_registerCommands(){if(!this._info)return;const e=this._info.display_name||this.name;Z(this,`plugin-detail:${this.kind}:${this.name}`,[{id:`remove:${this.kind}:${this.name}`,category:"Plugin",label:`Remove ${e}`,action:()=>this._remove()}])}async _toggle(){var s;const e=((s=this._info)==null?void 0:s.enabled)!==!1?"disable":"enable",{data:t}=await v(this.apiBase,`/plugins/${this.kind}/${this.name}/${e}`,{method:"POST"});this._message={type:t!=null&&t.ok?"success":"error",text:(t==null?void 0:t.message)||`${e} failed`},await this._fetchInfo(),this.dispatchEvent(new CustomEvent("plugin-state-changed",{bubbles:!0,composed:!0}))}async _sync(){this._syncing=!0,this._message=null;try{const e=await fetch(`${this.apiBase}/sync/${this.name}`,{method:"POST"});if(!e.ok){const m=await e.json().catch(()=>({}));this._message={type:"error",text:m.detail||`Sync failed (${e.status})`},this._syncing=!1;return}const t=e.body.getReader(),s=new TextDecoder;let a="",i="",r=!1;for(;;){const{done:m,value:y}=await t.read();if(m)break;const T=s.decode(y,{stream:!0});for(const f of T.split(`
`))f.startsWith("event: ")&&(a=f.slice(7).trim()),f.startsWith("data: ")&&(i=f.slice(6));a==="error"&&(r=!0)}let o="Sync complete";try{o=JSON.parse(i).message||o}catch{}this._message={type:r?"error":"success",text:o},r||await this._fetchInfo()}catch(e){this._message={type:"error",text:`Sync failed: ${e.message}`}}this._syncing=!1}async _remove(){const{data:e}=await v(this.apiBase,`/plugins/${this.kind}/${this.name}`,{method:"DELETE"});e!=null&&e.ok?(window.history.pushState({},"",`/settings/${this.kind}`),window.dispatchEvent(new PopStateEvent("popstate"))):this._message={type:"error",text:e.message||"Remove failed"}}_switchTab(e){this.activeTab=e;const t=`/settings/${this.kind}/${this.name}`,s=e==="details"?t:`${t}/${e}`;window.history.pushState({},"",s)}async _fetchPreview(e){if(this._selectedTable=e,!e){this._previewRows=null;return}this._previewLoading=!0,this._previewRows=await h(this.apiBase,`/db/preview/${this.name}/${e}?limit=100`),this._previewLoading=!1}_renderData(){const e=this._tables||[];return e.length===0?n`<p style="color:var(--shenas-text-muted,#888)">No tables synced yet.</p>`:n`
      <div class="data-toolbar">
        <select @change=${t=>this._fetchPreview(t.target.value)}>
          <option value="">Select a table</option>
          ${e.map(t=>n`<option value=${t.name} ?selected=${this._selectedTable===t.name}>${t.name}${t.rows?` (${t.rows})`:""}</option>`)}
        </select>
      ...
      ${this._previewRows&&this._previewRows.length>0?n`
        <table class="data-table">
          <thead><tr>${Object.keys(this._previewRows[0]).map(t=>n`<th>${t}</th>`)}</tr></thead>
          <tbody>${this._previewRows.map(t=>n`
            <tr>${Object.values(t).map(s=>n`<td title="${s??""}">${s??""}</td>`)}</tr>
          `)}</tbody>
        </table>
    ...
    `:""}}d(M,"properties",{apiBase:{type:String,attribute:"api-base"},kind:{type:String},name:{type:String},activeTab:{type:String,attribute:"active-tab"},_info:{state:!0},_loading:{state:!0},_message:{state:!0},_tables:{state:!0},_syncing:{state:!0},_schemaTransforms:{state:!0},_selectedTable:{state:!0},_previewRows:{state:!0},_previewLoading:{state:!0}}),d(M,"styles",[x,H,E,se,p`
      :host {
        display: block;
      }
      .back {
    ...
    `}}d(W,"properties",{apiBase:{type:String,attribute:"api-base"},source:{type:String},_transforms:{state:!0},_loading:{state:!0},_editing:{state:!0},_editSql:{state:!0},_message:{state:!0},_previewRows:{state:!0},_creating:{state:!0},_newForm:{state:!0},_dbTables:{state:!0},_schemaTables:{state:!0}}),d(W,"styles",[X,x,B,E,p`
      :host {
        display: block;
      }
      .edit-panel {
