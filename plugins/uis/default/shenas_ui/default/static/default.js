...
`;class B extends _{constructor(){super(),this.loading=!1,this.empty=!1,this.loadingText="Loading...",this.emptyText="No data",this.displayName=""}updated(e){e.has("displayName")&&this.displayName&&this.dispatchEvent(new CustomEvent("page-title",{bubbles:!0,composed:!0,detail:{title:this.displayName}}))}render(){return this.loading?n`<p class="loading">${this.loadingText}</p>`:this.empty?n`<p class="empty">${this.emptyText}</p>`:n`<slot></slot>`}}c(B,"properties",{loading:{type:Boolean,reflect:!0},empty:{type:Boolean,reflect:!0},loadingText:{type:String,attribute:"loading-text"},emptyText:{type:String,attribute:"empty-text"},displayName:{type:String,attribute:"display-name"}}),c(B,"styles",[I,g`
    ...
    `]);customElements.define("shenas-page",B);class N extends _{constructor(){super(),this.enabled=!1,this.toggleable=!1}updated(){this.title=this.enabled?"Enabled":"Disabled"}render(){return n`<div class="track" @click=${this._onClick}><div class="knob"></div></div>`}_onClick(){this.toggleable&&this.dispatchEvent(new CustomEvent("toggle",{bubbles:!0,composed:!0}))}}c(N,"properties",{enabled:{type:Boolean,reflect:!0},toggleable:{type:Boolean,reflect:!0}}),c(N,"styles",g`
    :host {
  ...
  `);customElements.define("status-toggle",N);class O extends _{constructor(){super(),this.columns=[],this.rows=[],this.rowClass=null,this.actions=null,this.emptyText="No items",this.showAdd=!1}_onAdd(){this.dispatchEvent(new CustomEvent("add",{bubbles:!0,composed:!0}))}render(){const e=typeof this.actions=="function",t=this.showAdd?n`<div class="add-row"><button class="add-btn" title="Add" @click=${this._onAdd}>+</button></div>`:"";return!this.rows||this.rows.length===0?n`<p class="empty">${this.emptyText}</p>${t}`:n`
      ...
      ${t}
    `}}c(O,"properties",{columns:{type:Array},rows:{type:Array},rowClass:{type:Object},actions:{type:Object},emptyText:{type:String,attribute:"empty-text"},showAdd:{type:Boolean,attribute:"show-add"}}),c(O,"styles",[X,x,I,g`
      :host {
        display: block;
      }
      .mono {
    ...
    `]);customElements.define("shenas-data-list",O);class q extends _{constructor(){super(),this.title="",this.submitLabel="Save"}render(){return n`
    ...
    `}_onSubmit(){this.dispatchEvent(new CustomEvent("submit",{bubbles:!0,composed:!0}))}_onCancel(){this.dispatchEvent(new CustomEvent("cancel",{bubbles:!0,composed:!0}))}}c(q,"properties",{title:{type:String},submitLabel:{type:String,attribute:"submit-label"}}),c(q,"styles",[x,g`
      :host {
        display: block;
        margin: 1rem 0;
        padding: 1rem;
    ...
    `}}c(L,"properties",{apiBase:{type:String,attribute:"api-base"},allPlugins:{type:Object},schemaPlugins:{type:Object},_loading:{state:!0},_empty:{state:!0}}),c(L,"styles",[I,g`
      :host {
        display: block;
      }
      #cy {
    ...
    `}}c(A,"properties",{apiBase:{type:String,attribute:"api-base"},pipeName:{type:String,attribute:"pipe-name"},_fields:{state:!0},_instructions:{state:!0},_loading:{state:!0},_message:{state:!0},_needsMfa:{state:!0},_oauthUrl:{state:!0},_submitting:{state:!0},_stored:{state:!0}}),c(A,"styles",[x,P,C,g`
      :host {
        display: block;
      }
      .instructions {
    ...
    `}}c(K,"properties",{apiBase:{type:String,attribute:"api-base"},actions:{type:Array},_bindings:{state:!0},_recording:{state:!0},_recordedKey:{state:!0},_conflict:{state:!0},_loading:{state:!0},_filter:{state:!0}}),c(K,"styles",[x,C,I,g`
      :host {
        display: block;
      }
      .toolbar {
    ...
    `}_getOrCreateElement(t){if(!this._elementCache.has(t.name)){const s=document.createElement(t.tag);s.setAttribute("api-base",this.apiBase),this._elementCache.set(t.name,s)}return this._elementCache.get(t.name)}}c(U,"properties",{apiBase:{type:String,attribute:"api-base"},_components:{state:!0},_loading:{state:!0},_loadedScripts:{state:!0},_leftWidth:{state:!0},_rightWidth:{state:!0},_dbStatus:{state:!0},_inspectTable:{state:!0},_inspectRows:{state:!0},_paletteOpen:{state:!0},_paletteCommands:{state:!0},_navPaletteOpen:{state:!0},_navCommands:{state:!0},_tabs:{state:!0},_activeTabId:{state:!0}}),c(U,"styles",[V,I,g`
      :host {
        display: block;
        height: 100vh;
        color: var(--shenas-text, #222);
    ...
    `}_formatFreq(e){return e>=1440&&e%1440===0?`${e/1440}d`:e>=60&&e%60===0?`${e/60}h`:e>=1?`${e}m`:`${e*60}s`}_renderKind(e){var a;const t=this._plugins[e]||[],s=((a=w.find(i=>i.id===e))==null?void 0:a.label)||e;return n`
      <h3>${s}</h3>
      <shenas-data-list
...
`))f.startsWith("event: ")&&(a=f.slice(7).trim()),f.startsWith("data: ")&&(i=f.slice(6));a==="error"&&(r=!0)}let o="Sync complete";try{o=JSON.parse(i).message||o}catch{}this._message={type:r?"error":"success",text:o},r||await this._fetchInfo()}catch(e){this._message={type:"error",text:`Sync failed: ${e.message}`}}this._syncing=!1}async _runTransforms(){this._transforming=!0,this._message=null;try{const{data:e}=await m(this.apiBase,"mutation($s: String!) { runSchemaTransforms(schema: $s) }",{s:this.name}),t=e==null?void 0:e.runSchemaTransforms;(t==null?void 0:t.count)!=null?(this._message={type:"success",text:`Ran ${t.count} transform(s)`},await this._fetchInfo()):this._message={type:"error",text:"Transform failed"}}catch(e){this._message={type:"error",text:`Transform failed: ${e.message}`}}this._transforming=!1}async _flush(){this._message=null;try{const{data:e}=await m(this.apiBase,"mutation($s: String!) { flushSchema(schemaPlugin: $s) }",{s:this.name}),t=e==null?void 0:e.flushSchema;(t==null?void 0:t.rows_deleted)!=null?(this._message={type:"success",text:`Flushed ${t.rows_deleted} rows`},await this._fetchInfo()):this._message={type:"error",text:"Flush failed"}}catch(e){this._message={type:"error",text:`Flush failed: ${e.message}`}}}async _remove(){var t,s;const{data:e}=await m(this.apiBase,"mutation($k: String!, $n: String!) { removePlugin(kind: $k, name: $n) { ok message } }",{k:this.kind,n:this.name});(t=e==null?void 0:e.removePlugin)!=null&&t.ok?(window.history.pushState({},"",`/settings/${this.kind}`),window.dispatchEvent(new PopStateEvent("popstate"))):this._message={type:"error",text:((s=e==null?void 0:e.removePlugin)==null?void 0:s.message)||"Remove failed"}}_switchTab(e){this.activeTab=e;const t=`/settings/${this.kind}/${this.name}`,s=e==="details"?t:`${t}/${e}`;window.history.pushState({},"",s)}async _fetchPreview(e){if(this._selectedTable=e,!e){this._previewRows=null;return}this._previewLoading=!0;const t=this.kind==="schema"?"metrics":this.name;this._previewRows=await E(this.apiBase,`SELECT * FROM "${t}"."${e}" ORDER BY 1 DESC LIMIT 100`),this._previewLoading=!1}_renderData(){var t,s;const e=this._tables||[];if(e.length===0)return n`<p style="color:var(--shenas-text-muted,#888)">No tables synced yet.</p>`;if(!this._selectedTable){const a=(t=this._info)==null?void 0:t.primary_table,i=a&&e.some(r=>r.name===a)?a:(s=e[0])==null?void 0:s.name;i&&this._fetchPreview(i)}return n`
    ...
    `:""}}c(W,"properties",{apiBase:{type:String,attribute:"api-base"},kind:{type:String},name:{type:String},activeTab:{type:String,attribute:"active-tab"},dbStatus:{type:Object},schemaPlugins:{type:Object},_info:{state:!0},_loading:{state:!0},_message:{state:!0},_tables:{state:!0},_syncing:{state:!0},_transforming:{state:!0},_schemaTransforms:{state:!0},_selectedTable:{state:!0},_previewRows:{state:!0},_previewLoading:{state:!0}}),c(W,"styles",[x,V,C,ie,g`
      :host {
        display: block;
      }
      .back {
    ...
    `}}c(H,"properties",{apiBase:{type:String,attribute:"api-base"},source:{type:String},_transforms:{state:!0},_loading:{state:!0},_editing:{state:!0},_editSql:{state:!0},_message:{state:!0},_previewRows:{state:!0},_creating:{state:!0},_newForm:{state:!0},_dbTables:{state:!0},_schemaTables:{state:!0}}),c(H,"styles",[X,x,P,C,g`
      :host {
        display: block;
      }
      .edit-panel {
    ...
    `]);customElements.define("shenas-transforms",H);
