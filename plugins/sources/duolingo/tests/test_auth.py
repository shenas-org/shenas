...
class TestBuildClient:
    def test_no_jwt_raises(self, source: DuolingoSource, auth_mock) -> None:
    ...
    def test_returns_client_with_jwt(self, mock_cls: MagicMock, source: DuolingoSource, auth_mock) -> None:
...
class TestAuthenticate:
    def test_missing_token(self, source: DuolingoSource) -> None:
        with pytest.raises(ValueError, match="jwt_token is required"):
            source.authenticate({})

    @patch("shenas_sources.duolingo.client.DuolingoClient")
    def test_success(self, mock_cls: MagicMock, source: DuolingoSource, auth_mock) -> None:
        mock_client = MagicMock()
        mock_cls.return_value = mock_client

        source.authenticate({"jwt_token": "tok-123"})
    ...
    @patch("shenas_sources.duolingo.client.DuolingoClient")
    def test_invalid_token(self, mock_cls: MagicMock, source: DuolingoSource) -> None:
        mock_client = MagicMock()
        mock_client.get_user.side_effect = Exception("401 Unauthorized")
        mock_cls.return_value = mock_client

        with pytest.raises(Exception, match="401"):
            source.authenticate({"jwt_token": "bad-token"})
        mock_client.close.assert_called_once()
