...
def pipe() -> DuolingoSource:
    p = DuolingoSource.__new__(DuolingoSource)
...
class TestBuildClient:
    def test_no_jwt_raises(self, pipe: DuolingoSource) -> None:
    ...
    def test_returns_client_with_jwt(self, mock_cls: MagicMock, pipe: DuolingoSource) -> None:
...
class TestAuthenticate:
    ...
    @patch("shenas_sources.duolingo.client.DuolingoClient")
    def test_success(self, mock_cls: MagicMock, pipe: DuolingoSource) -> None:
        mock_client = MagicMock()
        mock_cls.return_value = mock_client
        ...
        pipe._auth_store.set.assert_called_once_with(DuolingoSource.Auth, jwt_token="tok-123")
