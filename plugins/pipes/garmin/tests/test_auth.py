...
@pytest.fixture
def pipe() -> GarminPipe:
    p = GarminPipe.__new__(GarminPipe)
...
class TestBuildClient:
    def test_no_tokens_raises(self, pipe: GarminPipe) -> None:
        pipe._auth_store.get.return_value = None
        with pytest.raises(RuntimeError, match="No valid tokens"):
            pipe.build_client()

    def test_empty_tokens_raises(self, pipe: GarminPipe) -> None:
        pipe._auth_store.get.return_value = {"tokens": None}
        with pytest.raises(RuntimeError, match="No valid tokens"):
            pipe.build_client()

    @patch("garminconnect.Garmin")
    def test_valid_tokens_login_success(self, mock_garmin_cls: MagicMock, pipe: GarminPipe) -> None:
    ...
    @patch("garminconnect.Garmin")
    def test_login_fails_raises(self, mock_garmin_cls: MagicMock, pipe: GarminPipe) -> None:
