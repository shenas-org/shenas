...
class Table:
    ...
    def table_metadata(cls) -> dict[str, Any]:
        ...
        """
        # Don't pass `globalns=` here. ``get_type_hints`` walks the MRO
        # and resolves each base class's annotations against THAT base's
        # own ``__module__``, which is what we want -- ``ClassVar`` (and
        # other typing imports) live in whichever file declared them,
        # not necessarily in the leaf subclass's module. Forcing
        # ``globalns=vars(leaf_module)`` makes intermediate-base
        # annotations like ``SourceConfig._abstract: ClassVar[bool]``
        # resolve in the leaf's namespace, which often doesn't import
        # ``ClassVar`` and crashes with a NameError at request time.
...
class UserTable(Table):
    """Base for per-user system tables where ``user_id`` is part of the PK.

    Concrete subclasses must include ``user_id`` in ``table_pk``.

    Provides user-scoped CRUD operations that operate on rows belonging to a
    specific user, as determined by ``user_id``.  These replace the repeated
    raw SQL that would otherwise appear in every per-user table:

    - :meth:`read_rows` -- SELECT ... WHERE user_id = ?
    - :meth:`upsert_row` -- INSERT ... ON CONFLICT (full PK) DO UPDATE
    - :meth:`delete_row` -- DELETE the row matching the full composite PK
    - :meth:`delete_rows` -- DELETE all rows for the given user

    For the upsert, fields whose value is ``None`` **and** whose ``Field``
    metadata carries a ``db_default`` are omitted from the INSERT column list
    so the database-side default applies.  Pass an explicit value to override
    (e.g. ``updated_at=datetime.now(tz).isoformat()`` to record a write time).
    """
    ...
    @classmethod
    def read_rows(cls, user_id: int, *, schema: str | None = None) -> list[dict[str, Any]]:
        """Return all rows for ``user_id`` as a list of dicts."""
        from app.db import cursor
    ...
    @classmethod
    def upsert_row(cls, user_id: int, *, schema: str | None = None, **kwargs: Any) -> None:
        """Insert or update the row identified by the full composite PK.

        Fields that are ``None`` and carry a ``db_default`` in their ``Field``
        metadata are excluded from the INSERT so the database assigns the
        default.  All other fields are included; the ON CONFLICT clause updates
        every non-PK included column to its ``excluded.*`` counterpart.
        """
        from app.db import cursor
        ...
        # Build the full row: field Python defaults first, then kwargs, then
        # enforce user_id (kwargs must not override it via a different path).
        row: dict[str, Any] = {}
        for f in dataclasses.fields(cls):
            if f.default is not dataclasses.MISSING:
                row[f.name] = f.default
            elif f.default_factory is not dataclasses.MISSING:  # type: ignore[misc]
                row[f.name] = f.default_factory()  # type: ignore[misc]
            else:
                row[f.name] = None
        row.update(kwargs)
        row["user_id"] = user_id

        # Skip None-valued fields that have a db_default so the DB default applies.
    ...
    @classmethod
    def delete_row(cls, user_id: int, *, schema: str | None = None, **pk_kwargs: Any) -> None:
        """Delete the single row identified by the full composite PK."""
        from app.db import cursor
    ...
    def delete_rows(cls, user_id: int, *, schema: str | None = None) -> None:
        """Delete all rows belonging to ``user_id``."""
        from app.db import cursor
