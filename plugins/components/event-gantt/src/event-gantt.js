...
import { query, arrowToRows } from "./arrow-client.js";
...
export class ShenasEventGantt extends LitElement {
  ...
  async _fetchEvents() {
    ...
    try {
      ...
      this._events = arrowToRows(table);
    } catch (e) {
  ...
  render() {
    ...
    const categories = [...new Set(this._events.map((e) => e.category).filter(Boolean))];
  ...
  _renderBar(ev) {
    ...
    let durationHours = (ev.duration_min || 30) / 60;
    if (ev.end_at) {
      const end = new Date(Number(ev.end_at) / 1000);
      durationHours = (end - ev._start) / 3600000;
      ev._end = end;
    }
    ...
    return html`
      <div
        ...
        @mouseenter=${(e) => this._onBarEnter(ev, e)}
        @mousemove=${(e) => this._onBarMove(e)}
