...
def zurich_offset(dt_utc: datetime) -> timezone:
    ...
    cest_start = datetime(year, 3, _last_sunday_of_month(year, 3).day, 1, 0, 0, tzinfo=UTC)
    # CET resumes last Sunday of October at 01:00 UTC
    cet_start = datetime(year, 10, _last_sunday_of_month(year, 10).day, 1, 0, 0, tzinfo=UTC)
...
def rewrite_commit_dates(base: str | None, dry_run: bool = False) -> int:  # noqa: PLR0912, PLR0915
    """
    ...
    if base is None:
        ...
        ref_range = "HEAD" if not parent else f"{parent}..HEAD"
