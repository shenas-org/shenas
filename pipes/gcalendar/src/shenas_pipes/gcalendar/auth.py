"""Google Calendar OAuth2 token management via OS keyring.

Shares the same OAuth client credentials as the Gmail pipe but with
calendar-specific scopes and token storage.
"""
...
KEYRING_KEY = "gcalendar_token"
SCOPES = ["https://www.googleapis.com/auth/calendar.readonly"]

# Same OAuth client as Gmail (Desktop app -- safe to embed)
GCALENDAR_CLIENT_ID = "232211553387-3c4sog0fokns7ri2o6oj8d3s5v3r9jh6.apps.googleusercontent.com"
GCALENDAR_CLIENT_SECRET = "REDACTED_GOOGLE_OAUTH_CLIENT_SECRET"

AUTH_FIELDS = []  # OAuth browser flow, no manual credentials needed


def _get_client_config() -> dict:
    client_id = os.environ.get("SHENAS_GCALENDAR_CLIENT_ID", GCALENDAR_CLIENT_ID)
    client_secret = os.environ.get("SHENAS_GCALENDAR_CLIENT_SECRET", GCALENDAR_CLIENT_SECRET)
    return {
        "installed": {
            "client_id": client_id,
...
def build_client(run_auth_flow: bool = False):  # noqa: ANN201
    """Build a Google Calendar API service from keyring tokens or OAuth flow."""
    ...
    raise RuntimeError("No valid Google Calendar credentials. Run 'shenasctl pipe gcalendar auth' first.")


def authenticate(credentials: dict[str, str]) -> None:
    """Authenticate with Google Calendar via OAuth2.

    Same pattern as Gmail -- starts local server, returns URL to CLI.
    """
    import io
    import sys
    import threading
    ...
    if credentials.get("auth_complete") == "true" and "gcalendar" in _pending_mfa:
        state = _pending_mfa.pop("gcalendar")
        thread = state["thread"]
