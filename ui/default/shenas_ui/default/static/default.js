var I=Object.defineProperty;var F=(h,e,t)=>e in h?I(h,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):h[e]=t;var l=(h,e,t)=>F(h,typeof e!="symbol"?e+"":e,t);import{LitElement as u,css as c,html as i}from"lit";import O,{dagre as q}from"cytoscape";import{Router as L}from"@lit-labs/router";class v extends u{constructor(){super(),this.open=!1,this.commands=[],this._query="",this._filtered=[],this._selectedIndex=0}updated(e){e.has("open")&&this.open&&(this._query="",this._selectedIndex=0,this._filter(),requestAnimationFrame(()=>{const t=this.renderRoot.querySelector("input");t&&t.focus()})),e.has("commands")&&this._filter()}_filter(){const e=this._query.toLowerCase();e?this._filtered=this.commands.filter(t=>t.label.toLowerCase().includes(e)||t.category.toLowerCase().includes(e)||(t.description||"").toLowerCase().includes(e)):this._filtered=this.commands,this._selectedIndex>=this._filtered.length&&(this._selectedIndex=Math.max(0,this._filtered.length-1))}_onInput(e){this._query=e.target.value,this._selectedIndex=0,this._filter()}_onKeydown(e){if(e.key==="ArrowDown")e.preventDefault(),this._filtered.length>0&&(this._selectedIndex=Math.min(this._selectedIndex+1,this._filtered.length-1)),this._scrollToSelected();else if(e.key==="ArrowUp")e.preventDefault(),this._selectedIndex=Math.max(this._selectedIndex-1,0),this._scrollToSelected();else if(e.key==="Enter"){e.preventDefault();const t=this._filtered[this._selectedIndex];t&&this._execute(t)}else e.key==="Escape"&&this._close()}_scrollToSelected(){requestAnimationFrame(()=>{const e=this.renderRoot.querySelector(".item.selected");e&&e.scrollIntoView({block:"nearest"})})}_execute(e){this.dispatchEvent(new CustomEvent("execute",{detail:e,bubbles:!0,composed:!0}))}_close(){this.dispatchEvent(new CustomEvent("close",{bubbles:!0,composed:!0}))}render(){return this.open?i`
    ...
    `:i``}}l(v,"properties",{open:{type:Boolean,reflect:!0},commands:{type:Array},_query:{state:!0},_filtered:{state:!0},_selectedIndex:{state:!0}}),l(v,"styles",c`
    :host {
      display: none;
    }
    :host([open]) {
  ...
  `);customElements.define("shenas-command-palette",v);class w extends u{constructor(){super(),this.enabled=!1,this.toggleable=!1}updated(){this.title=this.enabled?"Enabled":"Disabled"}render(){return i`<div class="track" @click=${this._onClick}><div class="knob"></div></div>`}_onClick(){this.toggleable&&this.dispatchEvent(new CustomEvent("toggle",{bubbles:!0,composed:!0}))}}l(w,"properties",{enabled:{type:Boolean,reflect:!0},toggleable:{type:Boolean,reflect:!0}}),l(w,"styles",c`
    :host {
...
`;class x extends u{constructor(){super(),this.columns=[],this.rows=[],this.rowClass=null,this.actions=null,this.emptyText="No items",this.showAdd=!1}_onAdd(){this.dispatchEvent(new CustomEvent("add",{bubbles:!0,composed:!0}))}render(){const e=typeof this.actions=="function",t=this.showAdd?i`<div class="add-row"><button class="add-btn" title="Add" @click=${this._onAdd}>+</button></div>`:"";return!this.rows||this.rows.length===0?i`<p class="empty">${this.emptyText}</p>${t}`:i`
      ...
      ${t}
    `}}l(x,"properties",{columns:{type:Array},rows:{type:Array},rowClass:{type:Object},actions:{type:Object},emptyText:{type:String,attribute:"empty-text"},showAdd:{type:Boolean,attribute:"show-add"}}),l(x,"styles",[P,b,f,c`
      :host {
        display: block;
      }
      .mono {
    ...
    `]);customElements.define("shenas-data-list",x);class k extends u{constructor(){super(),this.title="",this.submitLabel="Save"}render(){return i`
    ...
    `}_onSubmit(){this.dispatchEvent(new CustomEvent("submit",{bubbles:!0,composed:!0}))}_onCancel(){this.dispatchEvent(new CustomEvent("cancel",{bubbles:!0,composed:!0}))}}l(k,"properties",{title:{type:String},submitLabel:{type:String,attribute:"submit-label"}}),l(k,"styles",[b,c`
      :host {
        display: block;
        margin: 1rem 0;
        padding: 1rem;
    ...
    `]);customElements.define("shenas-form-panel",k);let A=!1;class S extends u{constructor(){super(),this.apiBase="/api",this._loading=!0,this._empty=!1,this._cy=null,this._elements=null,this._resizeObserver=null}connectedCallback(){super.connectedCallback(),this._fetchData()}disconnectedCallback(){super.disconnectedCallback(),this._cy&&(this._cy.destroy(),this._cy=null),this._resizeObserver&&(this._resizeObserver.disconnect(),this._resizeObserver=null)}async _fetchData(){this._loading=!0;try{const[e,t,s,n]=await Promise.all([fetch(`${this.apiBase}/plugins/pipe`),fetch(`${this.apiBase}/plugins/schema`),fetch(`${this.apiBase}/transforms`),fetch(`${this.apiBase}/db/schema-plugins`)]),a=e.ok?await e.json():[],o=t.ok?await t.json():[],d=s.ok?await s.json():[],r=n.ok?await n.json():{};this._buildElements(a,o,d,r)}catch(e){console.error("Failed to fetch overview data:",e)}this._loading=!1}_buildElements(e,t,s,n){const a=[],o=new Set,d={};for(const[r,p]of Object.entries(n))for(const g of p)d[g]=r;for(const r of e){const p=`pipe:${r.name}`;o.add(p),a.push({data:{id:p,label:r.display_name||r.name,kind:"pipe"}})}for(const r of t){const p=`schema:${r.name}`;o.add(p),a.push({data:{id:p,label:r.display_name||r.name,kind:"schema"}})}for(const r of s){const p=`pipe:${r.source_plugin}`,g=d[r.target_duckdb_table],m=g?`schema:${g}`:null;if(!m||!o.has(p)||!o.has(m))continue;const y=r.description||`${r.source_duckdb_table} -> ${r.target_duckdb_table}`,D=y.length>30?y.slice(0,28)+"...":y;a.push({data:{id:`transform:${r.id}`,source:p,target:m,label:D,enabled:r.enabled?"yes":"no",sourcePlugin:r.source_plugin}})}this._elements=a,this._empty=s.length===0}_initCytoscape(){const e=this.renderRoot.querySelector("#cy");!e||!this._elements||(A||(O.use(q),A=!0),this._cy&&this._cy.destroy(),this._cy=O({container:e,elements:this._elements,style:[{selector:"node",style:{label:"data(label)","text-valign":"center","text-halign":"center","font-size":12,color:"#fff","text-wrap":"wrap","text-max-width":100,width:120,height:40,shape:"round-rectangle"}},{selector:'node[kind="pipe"]',style:{"background-color":"#4a90d9",cursor:"pointer"}},{selector:'node[kind="schema"]',style:{"background-color":"#66bb6a",cursor:"pointer"}},{selector:"edge",style:{"curve-style":"bezier","target-arrow-shape":"triangle","target-arrow-color":"#999","line-color":"#999",cursor:"pointer",width:2,label:"data(label)","font-size":9,color:"#888","text-rotation":"autorotate","text-margin-y":-8}},{selector:'edge[enabled="yes"]',style:{"line-style":"solid"}},{selector:'edge[enabled="no"]',style:{"line-style":"dashed","line-color":"#ccc","target-arrow-color":"#ccc",opacity:.5}}],layout:{name:"dagre",rankDir:"LR",nodeSep:60,rankSep:150,padding:30},userZoomingEnabled:!0,userPanningEnabled:!0,boxSelectionEnabled:!1}),this._cy.on("tap","node",t=>{const s=t.target.data(),n=s.id.substring(s.id.indexOf(":")+1),a=s.kind==="pipe"?`/settings/pipe/${n}`:`/settings/schema/${n}`;this.dispatchEvent(new CustomEvent("navigate",{bubbles:!0,composed:!0,detail:{path:a}}))}),this._cy.on("tap","edge",t=>{const s=t.target.data("sourcePlugin");s&&this.dispatchEvent(new CustomEvent("navigate",{bubbles:!0,composed:!0,detail:{path:`/settings/pipe/${s}`}}))}),this._resizeObserver&&this._resizeObserver.disconnect(),this._resizeObserver=new ResizeObserver(()=>{this._cy&&(this._cy.resize(),this._cy.fit(void 0,30))}),this._resizeObserver.observe(e))}firstUpdated(){!this._loading&&this._elements&&this._initCytoscape()}updated(e){e.has("_loading")&&!this._loading&&this._elements&&requestAnimationFrame(()=>this._initCytoscape())}render(){return this._loading?i`<p class="loading">Loading overview...</p>`:i`
      <div id="cy"></div>
      <div class="legend">
    ...
    `}}l(S,"properties",{apiBase:{type:String,attribute:"api-base"},_loading:{state:!0},_empty:{state:!0}}),l(S,"styles",[f,c`
      :host {
        display: flex;
        flex-direction: column;
        height: 100%;
    ...
    `}}l(C,"properties",{apiBase:{type:String,attribute:"api-base"},pipeName:{type:String,attribute:"pipe-name"},_fields:{state:!0},_instructions:{state:!0},_loading:{state:!0},_message:{state:!0},_needsMfa:{state:!0},_oauthUrl:{state:!0},_submitting:{state:!0}}),l(C,"styles",[b,_,f,c`
      :host {
        display: block;
      }
      .instructions {
    ...
    `}}l(E,"properties",{apiBase:{type:String,attribute:"api-base"},kind:{type:String},name:{type:String},_config:{state:!0},_loading:{state:!0},_message:{state:!0},_editing:{state:!0},_editValue:{state:!0}}),l(E,"styles",[b,_,f,c`
      :host {
        display: block;
      }
      .config-row {
    ...
    `]);customElements.define("shenas-config",E);class z extends u{constructor(){super();l(this,"_router",new L(this,[{path:"/",render:()=>this._renderDynamicHome()},{path:"/settings",render:()=>this._renderSettings("overview")},{path:"/settings/:kind",render:({kind:t})=>this._renderSettings(t)},{path:"/settings/:kind/:name",render:({kind:t,name:s})=>this._renderPluginDetail(t,s)},{path:"/settings/:kind/:name/config",render:({kind:t,name:s})=>this._renderPluginDetail(t,s,"config")},{path:"/settings/:kind/:name/auth",render:({kind:t,name:s})=>this._renderPluginDetail(t,s,"auth")},{path:"/:tab",render:({tab:t})=>this._renderDynamicTab(t)}]));this.apiBase="/api",this._components=[],this._loading=!0,this._loadedScripts=new Set,this._elementCache=new Map,this._leftWidth=160,this._rightWidth=220,this._dbStatus=null,this._inspectTable=null,this._inspectRows=null,this._paletteOpen=!1,this._paletteCommands=[]}connectedCallback(){super.connectedCallback(),this._fetchData(),this.addEventListener("plugin-state-changed",()=>this._refreshComponents()),this.addEventListener("inspect-table",t=>this._inspect(t.detail.schema,t.detail.table)),this.addEventListener("navigate",t=>this._router.goto(t.detail.path)),this._keyHandler=t=>{(t.ctrlKey||t.metaKey)&&t.key==="k"&&(t.preventDefault(),this._togglePalette())},document.addEventListener("keydown",this._keyHandler)}disconnectedCallback(){super.disconnectedCallback(),document.removeEventListener("keydown",this._keyHandler)}async _togglePalette(){if(this._paletteOpen){this._paletteOpen=!1;return}await this._buildCommands(),this._paletteOpen=!0}async _buildCommands(){const t=[];for(const a of this._components)t.push({id:`nav:${a.name}`,category:"Navigate",label:a.display_name||a.name,path:`/${a.name}`});t.push({id:"nav:settings",category:"Navigate",label:"Settings",path:"/settings"}),t.push({id:"nav:dataflow",category:"Navigate",label:"Data Flow",path:"/settings/overview"}),t.push({id:"nav:settings:pipe",category:"Navigate",label:"Settings > Pipes",path:"/settings/pipe"}),t.push({id:"nav:settings:schema",category:"Navigate",label:"Settings > Schemas",path:"/settings/schema"}),t.push({id:"nav:settings:component",category:"Navigate",label:"Settings > Components",path:"/settings/component"}),t.push({id:"nav:settings:ui",category:"Navigate",label:"Settings > UI",path:"/settings/ui"});let s=[],n=[];try{[s,n]=await Promise.all([this._fetch("/plugins/pipe"),this._fetch("/plugins/schema")]),s=s||[],n=n||[]}catch{}for(const a of s){const o=a.display_name||a.name;t.push({id:`nav:pipe:${a.name}`,category:"Pipe",label:o,description:"Open details",path:`/settings/pipe/${a.name}`}),t.push({id:`sync:${a.name}`,category:"Pipe",label:`Sync ${o}`,description:"Run sync now",action:()=>this._syncPipe(a.name)})}for(const a of n)t.push({id:`nav:schema:${a.name}`,category:"Schema",label:a.display_name||a.name,description:"Open details",path:`/settings/schema/${a.name}`});this._paletteCommands=t}async _syncPipe(t){try{await fetch(`${this.apiBase}/sync/${t}`,{method:"POST"})}catch{}}_executePaletteCommand(t){const s=t.detail;s.path?this._router.goto(s.path):s.action&&s.action(),this._paletteOpen=!1}async _refreshComponents(){this._components=await this._fetch("/components")||[]}async _fetchData(){this._loading=!0;try{const[t,s]=await Promise.all([this._fetch("/components"),this._fetch("/db/status")]);this._components=t||[],this._dbStatus=s}catch(t){console.error("Failed to fetch data:",t)}this._loading=!1}async _fetch(t){const s=await fetch(`${this.apiBase}${t}`);return s.ok?s.json():null}_activeTab(){return(window.location.pathname.replace(/^\/+/,"")||"").split("/")[0]||(this._components.length>0?this._components[0].name:"settings")}_startDrag(t){return s=>{s.preventDefault();const n=s.clientX,a=t==="left"?this._leftWidth:this._rightWidth,o=s.target;o.classList.add("dragging");const d=p=>{const g=t==="left"?p.clientX-n:n-p.clientX,m=Math.max(80,Math.min(400,a+g));t==="left"?this._leftWidth=m:this._rightWidth=m},r=()=>{o.classList.remove("dragging"),window.removeEventListener("mousemove",d),window.removeEventListener("mouseup",r)};window.addEventListener("mousemove",d),window.addEventListener("mouseup",r)}}render(){if(this._loading)return i`<p class="loading">Loading...</p>`;const t=this._activeTab();return i`
      <div class="layout">
    ...
    `}_renderKind(e){var n;const t=this._plugins[e]||[],s=((n=$.find(a=>a.id===e))==null?void 0:n.label)||e;return i`
      <h3>${s}</h3>
      <shenas-data-list
    ...
    `}}l(T,"properties",{apiBase:{type:String,attribute:"api-base"},activeKind:{type:String,attribute:"active-kind"},onNavigate:{type:Function},_plugins:{state:!0},_loading:{state:!0},_actionMessage:{state:!0},_installing:{state:!0}}),l(T,"styles",[b,j,_,f,c`
      :host {
        display: block;
        height: 100%;
      }
...
`))m.startsWith("event: ")&&(n=m.slice(7).trim()),m.startsWith("data: ")&&(a=m.slice(6));n==="error"&&(o=!0)}let d="Sync complete";try{d=JSON.parse(a).message||d}catch{}this._message={type:o?"error":"success",text:d},o||await this._fetchInfo()}catch(e){this._message={type:"error",text:`Sync failed: ${e.message}`}}this._syncing=!1}async _remove(){const t=await(await fetch(`${this.apiBase}/plugins/${this.kind}/${this.name}`,{method:"DELETE"})).json();t.ok?(window.history.pushState({},"",`/settings/${this.kind}`),window.dispatchEvent(new PopStateEvent("popstate"))):this._message={type:"error",text:t.message||"Remove failed"}}render(){if(this._loading)return i`<p class="loading">Loading...</p>`;if(!this._info)return i`<p>Plugin not found.</p>`;const e=this._info,t=e.enabled!==!1,s=`/settings/${this.kind}/${this.name}`;return i`
      ...
      <div class="title-row">
        ...
        <div class="title-actions">
          ${this.kind==="pipe"&&t?i`<button @click=${this._sync} ?disabled=${this._syncing}>${this._syncing?"Syncing...":"Sync"}</button>`:""}
      ...
      ${this.activeTab==="config"&&this._hasConfig?i`<shenas-config api-base="${this.apiBase}" kind="${this.kind}" name="${this.name}"></shenas-config>`:this.activeTab==="auth"&&this._hasAuth?i`<shenas-auth api-base="${this.apiBase}" pipe-name="${this.name}"></shenas-auth>`:this._renderDetails(e,t)}
    ...
    `:""}}l(B,"properties",{apiBase:{type:String,attribute:"api-base"},kind:{type:String},name:{type:String},activeTab:{type:String,attribute:"active-tab"},_info:{state:!0},_loading:{state:!0},_message:{state:!0},_hasConfig:{state:!0},_hasAuth:{state:!0},_tables:{state:!0},_syncing:{state:!0}}),l(B,"styles",[b,j,_,M,f,c`
      :host {
        display: block;
      }
      .back {
    ...
    `}}l(R,"properties",{apiBase:{type:String,attribute:"api-base"},source:{type:String},_transforms:{state:!0},_loading:{state:!0},_editing:{state:!0},_editSql:{state:!0},_message:{state:!0},_previewRows:{state:!0},_creating:{state:!0},_newForm:{state:!0},_dbTables:{state:!0},_schemaTables:{state:!0}}),l(R,"styles",[P,b,_,f,c`
      :host {
        display: block;
      }
      .edit-panel {
