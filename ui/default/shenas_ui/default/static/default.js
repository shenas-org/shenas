var N=Object.defineProperty;var F=(u,t,e)=>t in u?N(u,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):u[t]=e;var d=(u,t,e)=>F(u,typeof t!="symbol"?t+"":t,e);import{LitElement as g,css as m,html as a}from"lit";import j,{dagre as L}from"cytoscape";import{Router as q}from"@lit-labs/router";class $ extends g{constructor(){super(),this.open=!1,this.commands=[],this._query="",this._filtered=[],this._selectedIndex=0}updated(t){t.has("open")&&this.open&&(this._query="",this._selectedIndex=0,this._filter(),requestAnimationFrame(()=>{const e=this.renderRoot.querySelector("input");e&&e.focus()})),t.has("commands")&&this._filter()}_filter(){const t=this._query.toLowerCase();t?this._filtered=this.commands.filter(e=>e.label.toLowerCase().includes(t)||e.category.toLowerCase().includes(t)||(e.description||"").toLowerCase().includes(t)):this._filtered=this.commands,this._selectedIndex>=this._filtered.length&&(this._selectedIndex=Math.max(0,this._filtered.length-1))}_onInput(t){this._query=t.target.value,this._selectedIndex=0,this._filter()}_onKeydown(t){if(t.key==="ArrowDown")t.preventDefault(),this._filtered.length>0&&(this._selectedIndex=Math.min(this._selectedIndex+1,this._filtered.length-1)),this._scrollToSelected();else if(t.key==="ArrowUp")t.preventDefault(),this._selectedIndex=Math.max(this._selectedIndex-1,0),this._scrollToSelected();else if(t.key==="Enter"){t.preventDefault();const e=this._filtered[this._selectedIndex];e&&this._execute(e)}else t.key==="Escape"&&this._close()}_scrollToSelected(){requestAnimationFrame(()=>{const t=this.renderRoot.querySelector(".item.selected");t&&t.scrollIntoView({block:"nearest"})})}_execute(t){this.dispatchEvent(new CustomEvent("execute",{detail:t,bubbles:!0,composed:!0}))}_close(){this.dispatchEvent(new CustomEvent("close",{bubbles:!0,composed:!0}))}render(){return this.open?a`
      ...
      <div class="panel">
        ...
        <div class="results">
          ${this._filtered.length===0?a`<div class="empty">No matching commands</div>`:this._filtered.map((t,e)=>a`
                  <div
                  ...
                  >
                    ...
                    ${t.description?a`<span class="item-desc">${t.description}</span>`:""}
                  </div>
                `)}
        </div>
      </div>
  ...
  `);customElements.define("shenas-command-palette",$);class w extends g{constructor(){super(),this.enabled=!1,this.toggleable=!1}updated(){this.title=this.enabled?"Enabled":"Disabled"}render(){return a`<div class="track" @click=${this._onClick}><div class="knob"></div></div>`}_onClick(){this.toggleable&&this.dispatchEvent(new CustomEvent("toggle",{bubbles:!0,composed:!0}))}}d(w,"properties",{enabled:{type:Boolean,reflect:!0},toggleable:{type:Boolean,reflect:!0}}),d(w,"styles",m`
    :host {
  ...
  `);customElements.define("status-toggle",w);const R=m`
  table {
    width: 100%;
...
`;class x extends g{constructor(){super(),this.columns=[],this.rows=[],this.rowClass=null,this.actions=null,this.emptyText="No items",this.showAdd=!1}_onAdd(){this.dispatchEvent(new CustomEvent("add",{bubbles:!0,composed:!0}))}render(){const t=typeof this.actions=="function",e=this.showAdd?a`<div class="add-row"><button class="add-btn" title="Add" @click=${this._onAdd}>+</button></div>`:"";return!this.rows||this.rows.length===0?a`<p class="empty">${this.emptyText}</p>${e}`:a`
      <table>
        <thead>
          <tr>
        ...
        <tbody>
          ${this.rows.map(s=>a`
              <tr class="${this.rowClass?this.rowClass(s):""}">
                ...
                `)}
                ${t?a`<td class="actions-cell">${this.actions(s)}</td>`:""}
              </tr>
            `)}
        </tbody>
      </table>
      ${e}
    `}}d(x,"properties",{columns:{type:Array},rows:{type:Array},rowClass:{type:Object},actions:{type:Object},emptyText:{type:String,attribute:"empty-text"},showAdd:{type:Boolean,attribute:"show-add"}}),d(x,"styles",[R,v,f,m`
      :host {
        display: block;
      }
      .mono {
    ...
    `]);customElements.define("shenas-data-list",x);class k extends g{constructor(){super(),this.title="",this.submitLabel="Save"}render(){return a`
    ...
    `}_onSubmit(){this.dispatchEvent(new CustomEvent("submit",{bubbles:!0,composed:!0}))}_onCancel(){this.dispatchEvent(new CustomEvent("cancel",{bubbles:!0,composed:!0}))}}d(k,"properties",{title:{type:String},submitLabel:{type:String,attribute:"submit-label"}}),d(k,"styles",[v,m`
      :host {
        display: block;
        margin: 1rem 0;
        padding: 1rem;
    ...
    `]);customElements.define("shenas-form-panel",k);let I=!1;class C extends g{constructor(){super(),this.apiBase="/api",this._loading=!0,this._empty=!1,this._cy=null,this._elements=null,this._resizeObserver=null}connectedCallback(){super.connectedCallback(),this._fetchData()}disconnectedCallback(){super.disconnectedCallback(),this._cy&&(this._cy.destroy(),this._cy=null),this._resizeObserver&&(this._resizeObserver.disconnect(),this._resizeObserver=null)}async _fetchData(){this._loading=!0;try{const[t,e,s,i]=await Promise.all([fetch(`${this.apiBase}/plugins/pipe`),fetch(`${this.apiBase}/plugins/schema`),fetch(`${this.apiBase}/transforms`),fetch(`${this.apiBase}/db/schema-plugins`)]),n=t.ok?await t.json():[],r=e.ok?await e.json():[],l=s.ok?await s.json():[],o=i.ok?await i.json():{};this._buildElements(n,r,l,o)}catch(t){console.error("Failed to fetch overview data:",t)}this._loading=!1}_buildElements(t,e,s,i){const n=[],r=new Set,l={};for(const[o,p]of Object.entries(i))for(const c of p)l[c]=o;for(const o of t){const p=`pipe:${o.name}`;r.add(p),n.push({data:{id:p,label:o.display_name||o.name,kind:"pipe",enabled:o.enabled!==!1?"yes":"no"}})}for(const o of e){const p=`schema:${o.name}`;r.add(p),n.push({data:{id:p,label:o.display_name||o.name,kind:"schema",enabled:o.enabled!==!1?"yes":"no"}})}for(const o of s){const p=`pipe:${o.source_plugin}`,c=l[o.target_duckdb_table],h=c?`schema:${c}`:null;if(!h||!r.has(p)||!r.has(h))continue;const b=o.description||`${o.source_duckdb_table} -> ${o.target_duckdb_table}`,A=b.length>30?b.slice(0,28)+"...":b;n.push({data:{id:`transform:${o.id}`,source:p,target:h,label:A,enabled:o.enabled?"yes":"no",sourcePlugin:o.source_plugin}})}this._elements=n,this._empty=s.length===0}_initCytoscape(){const t=this.renderRoot.querySelector("#cy");!t||!this._elements||(I||(j.use(L),I=!0),this._cy&&this._cy.destroy(),this._cy=j({container:t,elements:this._elements,style:[{selector:"node",style:{label:"data(label)","text-valign":"center","text-halign":"center","font-size":12,color:"#fff","text-wrap":"wrap","text-max-width":100,width:120,height:40,shape:"round-rectangle"}},{selector:'node[kind="pipe"]',style:{"background-color":"#4a90d9",cursor:"pointer"}},{selector:'node[kind="schema"]',style:{"background-color":"#66bb6a",cursor:"pointer"}},{selector:'node[enabled="no"]',style:{opacity:.4,"border-width":2,"border-color":"#999","border-style":"dashed"}},{selector:"edge",style:{"curve-style":"bezier","target-arrow-shape":"triangle","target-arrow-color":"#999","line-color":"#999",cursor:"pointer",width:2,label:"data(label)","font-size":9,color:"#888","text-rotation":"autorotate","text-margin-y":-8}},{selector:'edge[enabled="yes"]',style:{"line-style":"solid"}},{selector:'edge[enabled="no"]',style:{"line-style":"dashed","line-color":"#ccc","target-arrow-color":"#ccc",opacity:.5}}],layout:{name:"dagre",rankDir:"LR",nodeSep:60,rankSep:150,padding:30},userZoomingEnabled:!0,userPanningEnabled:!0,boxSelectionEnabled:!1}),this._cy.on("tap","node",e=>{const s=e.target.data(),i=s.id.substring(s.id.indexOf(":")+1),n=s.kind==="pipe"?`/settings/pipe/${i}`:`/settings/schema/${i}`;this.dispatchEvent(new CustomEvent("navigate",{bubbles:!0,composed:!0,detail:{path:n}}))}),this._cy.on("tap","edge",e=>{const s=e.target.data("sourcePlugin");s&&this.dispatchEvent(new CustomEvent("navigate",{bubbles:!0,composed:!0,detail:{path:`/settings/pipe/${s}`}}))}),this._resizeObserver&&this._resizeObserver.disconnect(),this._resizeObserver=new ResizeObserver(()=>{this._cy&&(this._cy.resize(),this._cy.fit(void 0,30))}),this._resizeObserver.observe(t))}firstUpdated(){!this._loading&&this._elements&&this._initCytoscape()}updated(t){t.has("_loading")&&!this._loading&&this._elements&&requestAnimationFrame(()=>this._initCytoscape())}render(){return this._loading?a`<p class="loading">Loading overview...</p>`:a`
      <div id="cy"></div>
      <div class="legend">
    ...
    `}}d(T,"properties",{apiBase:{type:String,attribute:"api-base"},pipeName:{type:String,attribute:"pipe-name"},_fields:{state:!0},_instructions:{state:!0},_loading:{state:!0},_message:{state:!0},_needsMfa:{state:!0},_oauthUrl:{state:!0},_submitting:{state:!0}}),d(T,"styles",[v,y,f,m`
      :host {
        display: block;
      }
      .instructions {
        ...
        ${e?a`
            ...
            <div class="config-detail">
              <div class="config-value ${t.value?"":"empty"}"
                ...
                title="Click to edit"
              >${t.value||"not set"}</div>
              ${t.description?a`<div class="config-desc">${t.description}</div>`:""}
            </div>`}
      </div>
    `}}d(S,"properties",{apiBase:{type:String,attribute:"api-base"},kind:{type:String},name:{type:String},_config:{state:!0},_loading:{state:!0},_message:{state:!0},_editing:{state:!0},_editValue:{state:!0}}),d(S,"styles",[v,y,f,m`
      :host {
        display: block;
      }
      .config-row {
    ...
    `]);customElements.define("shenas-config",S);const _=[{id:"pipe",label:"Pipes"},{id:"schema",label:"Schemas"},{id:"component",label:"Components"},{id:"ui",label:"UI"},{id:"theme",label:"Themes"}];class E extends g{constructor(){super();d(this,"_router",new q(this,[{path:"/",render:()=>this._renderDynamicHome()},{path:"/settings",render:()=>this._renderSettings("overview")},{path:"/settings/:kind",render:({kind:e})=>this._renderSettings(e)},{path:"/settings/:kind/:name",render:({kind:e,name:s})=>this._renderPluginDetail(e,s)},{path:"/settings/:kind/:name/config",render:({kind:e,name:s})=>this._renderPluginDetail(e,s,"config")},{path:"/settings/:kind/:name/auth",render:({kind:e,name:s})=>this._renderPluginDetail(e,s,"auth")},{path:"/:tab",render:({tab:e})=>this._renderDynamicTab(e)}]));this.apiBase="/api",this._components=[],this._loading=!0,this._loadedScripts=new Set,this._elementCache=new Map,this._leftWidth=160,this._rightWidth=220,this._dbStatus=null,this._inspectTable=null,this._inspectRows=null,this._paletteOpen=!1,this._paletteCommands=[],this._navPaletteOpen=!1,this._navCommands=[],this._registeredCommands=new Map,this._tabs=[],this._activeTabId=null}connectedCallback(){super.connectedCallback(),this._fetchData(),this.addEventListener("plugin-state-changed",()=>this._refreshComponents()),this.addEventListener("inspect-table",e=>this._inspect(e.detail.schema,e.detail.table)),this.addEventListener("navigate",e=>this._navigateTo(e.detail.path,e.detail.label)),this.addEventListener("register-command",e=>{const{componentId:s,commands:i}=e.detail;!i||i.length===0?this._registeredCommands.delete(s):this._registeredCommands.set(s,i)}),this._keyHandler=e=>{(e.ctrlKey||e.metaKey)&&e.key==="p"?(e.preventDefault(),this._togglePalette()):(e.ctrlKey||e.metaKey)&&e.key==="o"&&(e.preventDefault(),this._toggleNavPalette())},document.addEventListener("keydown",this._keyHandler)}disconnectedCallback(){super.disconnectedCallback(),document.removeEventListener("keydown",this._keyHandler)}_togglePalette(){if(this._paletteOpen){this._paletteOpen=!1;return}this._navPaletteOpen=!1,this._buildCommands(),this._paletteOpen=!0}async _toggleNavPalette(){if(this._navPaletteOpen){this._navPaletteOpen=!1;return}this._paletteOpen=!1,await this._buildNavCommands(),this._navPaletteOpen=!0}async _buildNavCommands(){const e=[];for(const i of this._components)e.push({id:`nav:${i.name}`,category:"Page",label:i.display_name||i.name,path:`/${i.name}`});e.push({id:"nav:dataflow",category:"Settings",label:"Data Flow",path:"/settings/overview"});for(const i of _)e.push({id:`nav:settings:${i.id}`,category:"Settings",label:i.label,path:`/settings/${i.id}`});let s=[];try{s=(await Promise.all(_.map(async n=>(await this._fetch(`/plugins/${n.id}`)||[]).map(l=>({...l,kind:n.id,kindLabel:n.label}))))).flat()}catch{}for(const i of s)e.push({id:`nav:${i.kind}:${i.name}`,category:i.kindLabel,label:i.display_name||i.name,path:`/settings/${i.kind}/${i.name}`});this._navCommands=e}async _registerGlobalCommands(){const e=[];try{for(const s of _){const i=await this._fetch(`/plugins/${s.id}`)||[];for(const n of i){const r=n.display_name||n.name,l=n.enabled!==!1;e.push({id:`toggle:${s.id}:${n.name}`,category:s.label,label:`${l?"Disable":"Enable"} ${r}`,action:async()=>{const o=l?"disable":"enable";await fetch(`${this.apiBase}/plugins/${s.id}/${n.name}/${o}`,{method:"POST"}),await this._registerGlobalCommands()}}),s.id==="pipe"&&l&&e.push({id:`sync:${n.name}`,category:"Pipe",label:`Sync ${r}`,action:()=>fetch(`${this.apiBase}/sync/${n.name}`,{method:"POST"})})}}e.push({id:"sync:all",category:"Pipe",label:"Sync All Pipes",action:()=>fetch(`${this.apiBase}/sync`,{method:"POST"})}),e.push({id:"seed:transforms",category:"Transform",label:"Seed Default Transforms",action:()=>fetch(`${this.apiBase}/transforms/seed`,{method:"POST"})})}catch{}this._registeredCommands.set("global",e)}_buildCommands(){const e=[];for(const s of this._registeredCommands.values())e.push(...s);this._paletteCommands=e}_executePaletteCommand(e){const s=e.detail;s.path?this._openTab(s.path,s.label):s.action&&s.action(),this._paletteOpen=!1,this._navPaletteOpen=!1}_navigateTo(e,s){if(this._tabs.length===0||!this._activeTabId){this._openTab(e,s);return}const i=s||this._labelForPath(e);this._tabs=this._tabs.map(n=>n.path===this._activeTabId?{path:e,label:i}:n),this._activeTabId=e,this._router.goto(e)}_openTab(e,s){const i=this._tabs.find(n=>n.path===e);i?this._activeTabId=i.path:(this._tabs=[...this._tabs,{path:e,label:s||this._labelForPath(e)}],this._activeTabId=e),this._router.goto(e)}async _addTab(){await this._buildNavCommands(),this._navPaletteOpen=!0}_closeTab(e){const s=this._tabs.findIndex(n=>n.path===e);if(s===-1)return;const i=this._tabs.filter(n=>n.path!==e);if(this._tabs=i,this._activeTabId===e)if(i.length>0){const n=i[Math.min(s,i.length-1)];this._activeTabId=n.path,this._router.goto(n.path)}else this._activeTabId=null}_switchTab(e){this._activeTabId=e,this._router.goto(e)}_labelForPath(e){const s=e.replace(/^\/+/,"");if(!s||s==="settings"||s==="settings/overview")return"Data Flow";const i=s.split("/");if(i[0]==="settings"){if(i.length===2){const r=_.find(l=>l.id===i[1]);return r?r.label:i[1]}if(i.length>=3)return i[2]}const n=this._components.find(r=>r.name===i[0]);return n?n.display_name||n.name:i[0]}async _refreshComponents(){this._components=await this._fetch("/components")||[]}async _fetchData(){this._loading=!0;try{const[s,i]=await Promise.all([this._fetch("/components"),this._fetch("/db/status")]);this._components=s||[],this._dbStatus=i}catch(s){console.error("Failed to fetch data:",s)}this._loading=!1,this._registerGlobalCommands();const e=window.location.pathname;e&&e!=="/"&&this._openTab(e)}async _fetch(e){const s=await fetch(`${this.apiBase}${e}`);return s.ok?s.json():null}_activeTab(){return(window.location.pathname.replace(/^\/+/,"")||"").split("/")[0]||(this._components.length>0?this._components[0].name:"settings")}_startDrag(e){return s=>{s.preventDefault();const i=s.clientX,n=e==="left"?this._leftWidth:this._rightWidth,r=s.target;r.classList.add("dragging");const l=p=>{const c=e==="left"?p.clientX-i:i-p.clientX,h=Math.max(80,Math.min(400,n+c));e==="left"?this._leftWidth=h:this._rightWidth=h},o=()=>{r.classList.remove("dragging"),window.removeEventListener("mousemove",l),window.removeEventListener("mouseup",o)};window.addEventListener("mousemove",l),window.addEventListener("mouseup",o)}}render(){if(this._loading)return a`<p class="loading">Loading...</p>`;const e=this._activeTab();return a`
      <div class="layout">
        ...
        <div class="panel-middle">
          ${this._tabs.length>0?a`
              <div class="tab-bar">
                ${this._tabs.map(s=>a`
                  <div class="tab-item ${s.path===this._activeTabId?"active":""}"
                    @click=${()=>this._switchTab(s.path)}>
                    <span>${s.label}</span>
                    <button class="tab-close" @click=${i=>{i.stopPropagation(),this._closeTab(s.path)}}>x</button>
                  </div>
                `)}
    ...
    ></shenas-settings>`}async _inspect(e,s){if(!/^[a-zA-Z_]\w*$/.test(e)||!/^[a-zA-Z_]\w*$/.test(s))return;const i=`${e}.${s}`;if(this._inspectTable===i){this._inspectTable=null,this._inspectRows=null;return}this._inspectTable=i,this._inspectRows=null;try{const n=await fetch(`${this.apiBase}/db/preview/${e}/${s}?limit=50`);this._inspectRows=n.ok?await n.json():[]}catch{this._inspectRows=[]}}_renderDbStats(){const e=this._dbStatus;return e?a`
      <div class="db-section">
        <div class="db-meta">
          ${e.size_mb!=null?a`<code>${e.size_mb} MB</code>`:a`<span>Not created</span>`}
        </div>
        ${(e.schemas||[]).map(s=>a`
            <h4>${s.name}</h4>
            ${s.tables.map(i=>a`
      ...
      ${this._inspectRows?this._inspectRows.length===0?a`<p class="empty" style="font-size:0.75rem">No rows</p>`:a`
            <div style="overflow-x: auto;">
              <table class="inspect-table">
                <thead>
                  <tr>${Object.keys(this._inspectRows[0]).map(e=>a`<th>${e}</th>`)}</tr>
                </thead>
                <tbody>
                  ${this._inspectRows.map(e=>a`<tr>${Object.keys(e).map(s=>a`<td title="${e[s]??""}">${e[s]??""}</td>`)}</tr>`)}
                </tbody>
              </table>
            </div>
          `:a`<p class="loading" style="font-size:0.75rem">Loading...</p>`}
    `}_getOrCreateElement(e){if(!this._elementCache.has(e.name)){const s=document.createElement(e.tag);s.setAttribute("api-base",this.apiBase),this._elementCache.set(e.name,s)}return this._elementCache.get(e.name)}}d(E,"properties",{apiBase:{type:String,attribute:"api-base"},_components:{state:!0},_loading:{state:!0},_loadedScripts:{state:!0},_leftWidth:{state:!0},_rightWidth:{state:!0},_dbStatus:{state:!0},_inspectTable:{state:!0},_inspectRows:{state:!0},_paletteOpen:{state:!0},_paletteCommands:{state:!0},_navPaletteOpen:{state:!0},_navCommands:{state:!0},_tabs:{state:!0},_activeTabId:{state:!0}}),d(E,"styles",[O,f,m`
      :host {
        display: block;
        height: 100vh;
        color: var(--shenas-text, #222);
    ...
    `]);customElements.define("shenas-app",E);class z extends g{constructor(){super(),this.apiBase="/api",this.activeKind="pipe",this.onNavigate=null,this._plugins={},this._loading=!0,this._actionMessage=null,this._installing=!1}connectedCallback(){super.connectedCallback(),this._fetchAll()}async _fetchAll(){this._loading=!0;const t={};await Promise.all(_.map(async({id:e})=>{const s=await fetch(`${this.apiBase}/plugins/${e}`);t[e]=s.ok?await s.json():[]})),this._plugins=t,this._loading=!1}async _togglePlugin(t,e,s){const i=s?"disable":"enable",r=await(await fetch(`${this.apiBase}/plugins/${t}/${e}/${i}`,{method:"POST"})).json();r.ok||(this._actionMessage={type:"error",text:r.message||`${i} failed`}),t==="theme"&&await this._applyActiveTheme(),await this._fetchAll()}async _applyActiveTheme(){const t=await fetch(`${this.apiBase}/theme`);if(!t.ok)return;const{css:e}=await t.json();let s=document.querySelector("link[data-shenas-theme]");e?(s||(s=document.createElement("link"),s.rel="stylesheet",s.setAttribute("data-shenas-theme",""),document.head.appendChild(s)),s.href=e):s&&s.remove()}async _install(t){var l,o;const e=this.shadowRoot.querySelector(`#install-${t}`),s=(l=e==null?void 0:e.value)==null?void 0:l.trim();if(!s)return;this._actionMessage=null;const r=(o=(await(await fetch(`${this.apiBase}/plugins/${t}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({names:[s],skip_verify:!0})})).json()).results)==null?void 0:o[0];r!=null&&r.ok?(this._actionMessage={type:"success",text:r.message},this._installing=!1,await this._fetchAll()):this._actionMessage={type:"error",text:(r==null?void 0:r.message)||"Install failed"}}render(){return this._loading?a`<p class="loading">Loading plugins...</p>`:a`
    ...
    `}_renderKind(t){var i;const e=this._plugins[t]||[],s=((i=_.find(n=>n.id===t))==null?void 0:i.label)||t;return a`
      <h3>${s}</h3>
      <shenas-data-list
        .columns=${[{label:"Name",render:n=>a`<a href="/settings/${t}/${n.name}">${n.display_name||n.name}</a>`},{key:"version",label:"Version",class:"mono"},{label:"Added",class:"mono",render:n=>n.added_at?n.added_at.slice(0,10):""},{label:"Status",render:n=>a`<status-toggle ?enabled=${n.enabled!==!1} toggleable @toggle=${()=>this._togglePlugin(t,n.name,n.enabled!==!1)}></status-toggle>`}]}
        .rows=${e}
    ...
    `}}d(z,"properties",{apiBase:{type:String,attribute:"api-base"},activeKind:{type:String,attribute:"active-kind"},onNavigate:{type:Function},_plugins:{state:!0},_loading:{state:!0},_actionMessage:{state:!0},_installing:{state:!0}}),d(z,"styles",[v,O,y,f,m`
      :host {
        display: block;
        height: 100%;
      }
    ...
    `]);customElements.define("shenas-settings",z);class P extends g{constructor(){super(),this.apiBase="/api",this.kind="",this.name="",this.activeTab="details",this._info=null,this._loading=!0,this._message=null,this._hasConfig=!1,this._hasAuth=!1,this._tables=[],this._syncing=!1,this._schemaTransforms=[]}willUpdate(t){(t.has("kind")||t.has("name"))&&this._fetchInfo()}async _fetchInfo(){var p;if(!this.kind||!this.name)return;this._loading=!0,this._message=null;const t=await fetch(`${this.apiBase}/plugins/${this.kind}/${this.name}/info`);this._info=t.ok?await t.json():null;const e=this.kind==="pipe"||this.kind==="schema",[s,i,n,r,l]=await Promise.all([fetch(`${this.apiBase}/config?kind=${this.kind}&name=${this.name}`),this.kind==="pipe"?fetch(`${this.apiBase}/auth/${this.name}/fields`):Promise.resolve(null),e?fetch(`${this.apiBase}/db/status`):Promise.resolve(null),this.kind==="schema"?fetch(`${this.apiBase}/db/schema-plugins`):Promise.resolve(null),this.kind==="schema"?fetch(`${this.apiBase}/transforms`):Promise.resolve(null)]);if(s.ok){const c=await s.json();this._hasConfig=c.length>0&&c[0].entries.length>0}if(i!=null&&i.ok){const c=await i.json();this._hasAuth=((p=c.fields)==null?void 0:p.length)>0||!!c.instructions}const o=r!=null&&r.ok?(await r.json())[this.name]||[]:[];if(n!=null&&n.ok){const c=await n.json();if(this.kind==="pipe"){const h=(c.schemas||[]).find(b=>b.name===this.name);this._tables=h?h.tables.filter(b=>!b.name.startsWith("_dlt_")):[]}else if(this.kind==="schema"){const h=(c.schemas||[]).find(b=>b.name==="metrics");this._tables=h?h.tables.filter(b=>o.includes(b.name)):[]}}if(l!=null&&l.ok){const c=await l.json();this._schemaTransforms=c.filter(h=>o.includes(h.target_duckdb_table))}this._loading=!1,this._registerCommands()}_registerCommands(){if(!this._info)return;const t=this._info.display_name||this.name,e=[{id:`remove:${this.kind}:${this.name}`,category:"Plugin",label:`Remove ${t}`,action:()=>this._remove()}];this.dispatchEvent(new CustomEvent("register-command",{bubbles:!0,composed:!0,detail:{componentId:`plugin-detail:${this.kind}:${this.name}`,commands:e}}))}async _toggle(){var i;const t=((i=this._info)==null?void 0:i.enabled)!==!1?"disable":"enable",s=await(await fetch(`${this.apiBase}/plugins/${this.kind}/${this.name}/${t}`,{method:"POST"})).json();this._message={type:s.ok?"success":"error",text:s.message||`${t} failed`},await this._fetchInfo(),this.dispatchEvent(new CustomEvent("plugin-state-changed",{bubbles:!0,composed:!0}))}async _sync(){this._syncing=!0,this._message=null;try{const t=await fetch(`${this.apiBase}/sync/${this.name}`,{method:"POST"});if(!t.ok){const o=await t.json().catch(()=>({}));this._message={type:"error",text:o.detail||`Sync failed (${t.status})`},this._syncing=!1;return}const e=t.body.getReader(),s=new TextDecoder;let i="",n="",r=!1;for(;;){const{done:o,value:p}=await e.read();if(o)break;const c=s.decode(p,{stream:!0});for(const h of c.split(`
`))h.startsWith("event: ")&&(i=h.slice(7).trim()),h.startsWith("data: ")&&(n=h.slice(6));i==="error"&&(r=!0)}let l="Sync complete";try{l=JSON.parse(n).message||l}catch{}this._message={type:r?"error":"success",text:l},r||await this._fetchInfo()}catch(t){this._message={type:"error",text:`Sync failed: ${t.message}`}}this._syncing=!1}async _remove(){const e=await(await fetch(`${this.apiBase}/plugins/${this.kind}/${this.name}`,{method:"DELETE"})).json();e.ok?(window.history.pushState({},"",`/settings/${this.kind}`),window.dispatchEvent(new PopStateEvent("popstate"))):this._message={type:"error",text:e.message||"Remove failed"}}render(){if(this._loading)return a`<p class="loading">Loading...</p>`;if(!this._info)return a`<p>Plugin not found.</p>`;const t=this._info,e=t.enabled!==!1,s=`/settings/${this.kind}/${this.name}`;return a`
      ...
      <div class="title-row">
        ...
        <div class="title-actions">
          ${this.kind==="pipe"&&e?a`<button @click=${this._sync} ?disabled=${this._syncing}>${this._syncing?"Syncing...":"Sync"}</button>`:""}
      ...
      ${this.activeTab==="config"&&this._hasConfig?a`<shenas-config api-base="${this.apiBase}" kind="${this.kind}" name="${this.name}"></shenas-config>`:this.activeTab==="auth"&&this._hasAuth?a`<shenas-auth api-base="${this.apiBase}" pipe-name="${this.name}"></shenas-auth>`:this._renderDetails(t,e)}
    ...
    `:""}}d(P,"properties",{apiBase:{type:String,attribute:"api-base"},kind:{type:String},name:{type:String},activeTab:{type:String,attribute:"active-tab"},_info:{state:!0},_loading:{state:!0},_message:{state:!0},_hasConfig:{state:!0},_hasAuth:{state:!0},_tables:{state:!0},_syncing:{state:!0},_schemaTransforms:{state:!0}}),d(P,"styles",[v,O,y,M,f,m`
      :host {
        display: block;
      }
      .back {
    ...
    `]);customElements.define("shenas-plugin-detail",P);const D="background:none;border:none;cursor:pointer;color:var(--shenas-text-faint, #aaa);font-size:0.7rem;padding:0 2px";class B extends g{constructor(){super(),this.apiBase="/api",this.source="",this._transforms=[],this._loading=!0,this._editing=null,this._editSql="",this._message=null,this._previewRows=null,this._creating=!1,this._newForm=this._emptyForm(),this._dbTables={},this._schemaTables={}}_emptyForm(){return{source_duckdb_table:"",target_duckdb_table:"",description:"",sql:""}}connectedCallback(){super.connectedCallback(),this._fetchAll()}async _fetchAll(){this._loading=!0;const t=this.source?`?source=${this.source}`:"",e=await fetch(`${this.apiBase}/transforms${t}`);this._transforms=e.ok?await e.json():[],this._loading=!1,this._registerCommands()}_registerCommands(){const t=[];for(const e of this._transforms){const s=e.description||`${e.source_duckdb_table} -> ${e.target_duckdb_table}`;t.push({id:`transform:toggle:${e.id}`,category:"Transform",label:`${e.enabled?"Disable":"Enable"} #${e.id}`,description:s,action:()=>this._toggle(e)}),e.is_default||t.push({id:`transform:delete:${e.id}`,category:"Transform",label:`Delete #${e.id}`,description:s,action:()=>this._delete(e)})}this.dispatchEvent(new CustomEvent("register-command",{bubbles:!0,composed:!0,detail:{componentId:`transforms:${this.source}`,commands:t}}))}_inspectTable(t,e){this.dispatchEvent(new CustomEvent("inspect-table",{bubbles:!0,composed:!0,detail:{schema:t,table:e}}))}async _toggle(t){const e=t.enabled?"disable":"enable";await fetch(`${this.apiBase}/transforms/${t.id}/${e}`,{method:"POST"}),await this._fetchAll()}async _delete(t){const s=await(await fetch(`${this.apiBase}/transforms/${t.id}`,{method:"DELETE"})).json();s.ok?(this._message={type:"success",text:s.message},await this._fetchAll()):this._message={type:"error",text:s.detail||s.message||"Delete failed"}}_startEdit(t){this._editing=t.id,this._editSql=t.sql,this._previewRows=null}_cancelEdit(){this._editing=null,this._editSql="",this._previewRows=null}async _saveEdit(){const t=await fetch(`${this.apiBase}/transforms/${this._editing}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({sql:this._editSql})});if(t.ok)this._message={type:"success",text:"Transform updated"},this._editing=null,await this._fetchAll();else{const e=await t.json();this._message={type:"error",text:e.detail||"Update failed"}}}async _startCreate(){this._creating=!0,this._newForm=this._emptyForm(),this._editing=null,this._previewRows=null;const[t,e]=await Promise.all([fetch(`${this.apiBase}/db/tables`),fetch(`${this.apiBase}/db/schema-tables`)]);this._dbTables=t.ok?await t.json():{},this._schemaTables=e.ok?await e.json():{}}_cancelCreate(){this._creating=!1,this._newForm=this._emptyForm()}_updateNewForm(t,e){this._newForm={...this._newForm,[t]:e}}async _saveCreate(){const t=this._newForm;if(!t.source_duckdb_table||!t.target_duckdb_table||!t.sql){this._message={type:"error",text:"Fill in all required fields"};return}const e=await fetch(`${this.apiBase}/transforms`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({source_duckdb_schema:this.source,source_duckdb_table:t.source_duckdb_table,target_duckdb_schema:"metrics",target_duckdb_table:t.target_duckdb_table,source_plugin:this.source,description:t.description,sql:t.sql})});if(e.ok)this._message={type:"success",text:"Transform created"},this._creating=!1,this._newForm=this._emptyForm(),await this._fetchAll();else{const s=await e.json();this._message={type:"error",text:s.detail||"Create failed"}}}async _preview(){const t=await fetch(`${this.apiBase}/transforms/${this._editing}/test?limit=5`,{method:"POST"});if(t.ok)this._previewRows=await t.json();else{const e=await t.json();this._message={type:"error",text:e.detail||"Preview failed"}}}render(){return this._loading?a`<p class="loading">Loading transforms...</p>`:a`
      ...
      <shenas-data-list
        ...
        .columns=${[{key:"id",label:"ID",class:"muted"},{label:"Source",class:"mono",render:t=>a`${t.source_duckdb_schema}.${t.source_duckdb_table} <button style=${D} title="Inspect table" @click=${()=>this._inspectTable(t.source_duckdb_schema,t.source_duckdb_table)}>&#9655;</button>`},{label:"Target",class:"mono",render:t=>a`${t.target_duckdb_schema}.${t.target_duckdb_table} <button style=${D} title="Inspect table" @click=${()=>this._inspectTable(t.target_duckdb_schema,t.target_duckdb_table)}>&#9655;</button>`},{label:"Description",render:t=>a`${t.description||""}${t.is_default?a`<span style="font-size:0.75rem;color:var(--shenas-text-muted, #888);background:var(--shenas-border-light, #f0f0f0);padding:1px 5px;border-radius:3px;margin-left:4px">default</span>`:""}`},{label:"Status",render:t=>a`<status-toggle ?enabled=${t.enabled} toggleable @toggle=${()=>this._toggle(t)}></status-toggle>`}]}
        ...
        .actions=${t=>a`
          ...
          ${t.is_default?"":a`<button class="danger" @click=${()=>this._delete(t)}>Delete</button>`}
        `}
        empty-text="No transforms"
      ></shenas-data-list>
    `}_renderCreateForm(){const t=this._newForm,e=this.source,s=this._dbTables[e]||[],i=Object.values(this._schemaTables||{}).flat();return a`
      <shenas-form-panel
        title="New transform"
        submit-label="Create"
        ...
        <div class="form-grid">
          <label>
            ...
            >
              ...
              ${s.map(n=>a`<option value=${n} ?selected=${t.source_duckdb_table===n}>${n}</option>`)}
            </select>
          </label>
          <label>
            Schema table
            ...
            >
              <option value="">-- select --</option>
              ${i.map(n=>a`<option value=${n} ?selected=${t.target_duckdb_table===n}>${n}</option>`)}
            </select>
          </label>
    ...
    `}}d(B,"properties",{apiBase:{type:String,attribute:"api-base"},source:{type:String},_transforms:{state:!0},_loading:{state:!0},_editing:{state:!0},_editSql:{state:!0},_message:{state:!0},_previewRows:{state:!0},_creating:{state:!0},_newForm:{state:!0},_dbTables:{state:!0},_schemaTables:{state:!0}}),d(B,"styles",[R,v,y,f,m`
      :host {
        display: block;
      }
      .edit-panel {
