var u=Object.defineProperty;var f=(o,s,e)=>s in o?u(o,s,{enumerable:!0,configurable:!0,writable:!0,value:e}):o[s]=e;var l=(o,s,e)=>f(o,typeof s!="symbol"?s+"":s,e);import{LitElement as g,css as b,html as i}from"lit";import{Router as _}from"@lit-labs/router";class c extends g{constructor(){super();l(this,"_router",new _(this,[{path:"/",render:()=>this._renderDb()},{path:"/database",render:()=>this._renderDb()},{path:"/pipes",render:()=>this._renderPipes()},{path:"/settings",render:()=>this._renderSettings("pipe")},{path:"/settings/:kind",render:({kind:e})=>this._renderSettings(e)},{path:"/:tab",render:({tab:e})=>this._renderDynamicTab(e)}]));this.apiBase="/api",this._pipes=[],this._dbStatus=null,this._components=[],this._loading=!0,this._loadedScripts=new Set,this._elementCache=new Map}connectedCallback(){super.connectedCallback(),this._fetchData()}async _fetchData(){this._loading=!0;try{const[e,t,a]=await Promise.all([this._fetch("/plugins/pipe"),this._fetch("/db/status"),this._fetch("/components")]);this._pipes=e||[],this._dbStatus=t,this._components=a||[]}catch(e){console.error("Failed to fetch data:",e)}this._loading=!1}async _fetch(e){const t=await fetch(`${this.apiBase}${e}`);return t.ok?t.json():null}_activeTab(){return(window.location.pathname.replace(/^\/+/,"")||"database").split("/")[0]}render(){if(this._loading)return i`<p class="loading">Loading...</p>`;const e=this._activeTab();return i`
      <div class="header">
    ...
    `}}l(c,"properties",{apiBase:{type:String,attribute:"api-base"},_pipes:{state:!0},_dbStatus:{state:!0},_components:{state:!0},_loading:{state:!0},_loadedScripts:{state:!0}}),l(c,"styles",b`
    :host {
      display: block;
      max-width: 960px;
      margin: 0 auto;
  ...
  `);customElements.define("shenas-app",c);const d=[{id:"pipe",label:"Pipes"},{id:"schema",label:"Schemas"},{id:"component",label:"Components"},{id:"ui",label:"UI"}];class p extends g{constructor(){super(),this.apiBase="/api",this.activeKind="pipe",this.onNavigate=null,this._plugins={},this._loading=!0,this._actionMessage=null}connectedCallback(){super.connectedCallback(),this._fetchAll()}async _fetchAll(){this._loading=!0;const s={};await Promise.all(d.map(async({id:e})=>{const t=await fetch(`${this.apiBase}/plugins/${e}`);s[e]=t.ok?await t.json():[]})),this._plugins=s,this._loading=!1}async _remove(s,e){this._actionMessage=null;const a=await(await fetch(`${this.apiBase}/plugins/${s}/${e}`,{method:"DELETE"})).json();a.ok?(this._actionMessage={type:"success",text:a.message},await this._fetchAll()):this._actionMessage={type:"error",text:a.message||"Remove failed"}}async _toggleEnabled(s,e,t){this._actionMessage=null;const a=t?"disable":"enable",r=await(await fetch(`${this.apiBase}/plugins/${s}/${e}/${a}`,{method:"POST"})).json();r.ok?(this._actionMessage={type:"success",text:r.message},await this._fetchAll()):this._actionMessage={type:"error",text:r.message||`${a} failed`}}async _install(s){var h,m;const e=this.shadowRoot.querySelector(`#install-${s}`),t=(h=e==null?void 0:e.value)==null?void 0:h.trim();if(!t)return;this._actionMessage=null;const r=(m=(await(await fetch(`${this.apiBase}/plugins/${s}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({names:[t],skip_verify:!0})})).json()).results)==null?void 0:m[0];r!=null&&r.ok?(this._actionMessage={type:"success",text:r.message},e.value="",await this._fetchAll()):this._actionMessage={type:"error",text:(r==null?void 0:r.message)||"Install failed"}}render(){return this._loading?i`<p class="loading">Loading plugins...</p>`:i`
                ...
                ${e.map(n=>i`
                    <tr style="${n.enabled===!1?"opacity: 0.5":""}">
                      ...
                      <td class="actions">
                        <button
                          class="action"
                          @click=${()=>this._toggleEnabled(s,n.name,n.enabled!==!1)}
                        >
    ...
    `}}l(p,"properties",{apiBase:{type:String,attribute:"api-base"},activeKind:{type:String,attribute:"active-kind"},onNavigate:{type:Function},_plugins:{state:!0},_loading:{state:!0},_actionMessage:{state:!0}}),l(p,"styles",b`
    :host {
      display: block;
    }
    .layout {
