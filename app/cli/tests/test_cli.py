...
class TestCheckSignature:
    def test_no_key(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("app.cli.commands.pkg.PUBLIC_KEY_PATH", tmp_path / "nonexistent.pub")
    ...
    def test_no_wheel(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        pub.write_text("fake")
        monkeypatch.setattr("app.cli.commands.pkg.PUBLIC_KEY_PATH", pub)
        monkeypatch.setattr("app.cli.commands.pkg.PACKAGES_DIR", tmp_path / "empty")
    ...
    def test_no_sig_file(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("app.cli.commands.pkg.PUBLIC_KEY_PATH", pub)
        monkeypatch.setattr("app.cli.commands.pkg.PACKAGES_DIR", pkg_dir)
        assert check_signature("shenas-pipe-test", "1.0.0") == "unsigned"
