...
from app.graphql.queries import _entity_rel_to_gql, _entity_row_to_gql, _transform_to_gql
...
class Mutation:
    ...
    def create_entity(self, entity_input: EntityCreateInput) -> GqlEntityType:
        """Create a new entity in the current user's graph."""
    ...
    def update_entity(self, uuid: str, entity_input: EntityUpdateInput) -> GqlEntityType | None:
        """Update an entity by UUID. Fields set to None are left untouched."""
        from app.entities import Entity
        ...
        if entity_input.name is not None:
            row.name = entity_input.name
        if entity_input.type is not None:
            row.type = entity_input.type
        if entity_input.description is not None:
            row.description = entity_input.description
        if entity_input.status is not None:
            row.status = entity_input.status
        if entity_input.birth_year is not None:
            row.birth_year = entity_input.birth_year
        row.save()
        return _entity_row_to_gql(row)

    @strawberry.mutation
    def delete_entity(self, uuid: str) -> OkType:
        """Delete an entity and every relationship it participates in."""
        from app.entities import Entity
        from app.models import OkResponse
    ...
    def create_entity_relationship(
        ...
        from_uuid: str,
        to_uuid: str,
        type: str,  # noqa: A002 -- matches the GraphQL field name
        description: str = "",
    ...
    def delete_entity_relationship(
        ...
        """Delete a specific directed edge between two entities."""
        from app.entities import EntityRelationship
        from app.models import OkResponse

        rel = EntityRelationship.find(from_uuid, to_uuid, type)
        if rel is not None:
            rel.delete()
        return OkType.from_pydantic(OkResponse(ok=True))  # ty: ignore[unresolved-attribute]
